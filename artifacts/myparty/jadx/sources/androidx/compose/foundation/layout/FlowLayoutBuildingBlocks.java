...
public final class FlowLayoutBuildingBlocks {
    ...
    public final WrapEllipsisInfo getWrapEllipsisInfo(WrapInfo wrapInfo, boolean hasNext, int lastContentLineIndex, int totalCrossAxisSize, int leftOverMainAxis, int nextIndexInLine) {
        ...
        wrapEllipsisInfoEllipsisInfo$foundation_layout_release.setPlaceEllipsisOnLastContentLine(lastContentLineIndex >= 0 && (nextIndexInLine == 0 || (leftOverMainAxis - IntIntPair.m367getFirstimpl(wrapEllipsisInfoEllipsisInfo$foundation_layout_release.getEllipsisSize()) >= 0 && nextIndexInLine < this.maxItemsInMainAxis)));
        return wrapEllipsisInfoEllipsisInfo$foundation_layout_release;
    }

    /* renamed from: getWrapInfo-OpUlnko, reason: not valid java name */
    public final WrapInfo m943getWrapInfoOpUlnko(boolean nextItemHasNext, int nextIndexInLine, long leftOver, IntIntPair nextSize, int lineIndex, int totalCrossAxisSize, int currentLineCrossAxisSize, boolean isWrappingRound, boolean isEllipsisWrap) {
        int i = totalCrossAxisSize + currentLineCrossAxisSize;
        if (nextSize == null) {
            return new WrapInfo(true, true);
        }
        if (this.overflow.getType$foundation_layout_release() != FlowLayoutOverflow.OverflowType.Visible && (lineIndex >= this.maxLines || IntIntPair.m368getSecondimpl(leftOver) - IntIntPair.m368getSecondimpl(nextSize.getPackedValue()) < 0)) {
            return new WrapInfo(true, true);
        }
        if (nextIndexInLine != 0 && (nextIndexInLine >= this.maxItemsInMainAxis || IntIntPair.m367getFirstimpl(leftOver) - IntIntPair.m367getFirstimpl(nextSize.getPackedValue()) < 0)) {
            if (isWrappingRound) {
                return new WrapInfo(true, true);
            }
            return new WrapInfo(true, m943getWrapInfoOpUlnko(nextItemHasNext, 0, IntIntPair.m363constructorimpl(Constraints.m6070getMaxWidthimpl(this.constraints), (IntIntPair.m368getSecondimpl(leftOver) - this.crossAxisSpacing) - currentLineCrossAxisSize), IntIntPair.m360boximpl(IntIntPair.m363constructorimpl(IntIntPair.m367getFirstimpl(nextSize.getPackedValue()) - this.mainAxisSpacing, IntIntPair.m368getSecondimpl(nextSize.getPackedValue()))), lineIndex + 1, i, 0, true, false).getIsLastItemInContainer());
        }
        int iMax = totalCrossAxisSize + Math.max(currentLineCrossAxisSize, IntIntPair.m368getSecondimpl(nextSize.getPackedValue()));
        IntIntPair intIntPairM948ellipsisSizeF35zmw$foundation_layout_release = isEllipsisWrap ? null : this.overflow.m948ellipsisSizeF35zmw$foundation_layout_release(nextItemHasNext, lineIndex, iMax);
        if (intIntPairM948ellipsisSizeF35zmw$foundation_layout_release != null) {
            intIntPairM948ellipsisSizeF35zmw$foundation_layout_release.getPackedValue();
            if (nextIndexInLine + 1 >= this.maxItemsInMainAxis || ((IntIntPair.m367getFirstimpl(leftOver) - IntIntPair.m367getFirstimpl(nextSize.getPackedValue())) - this.mainAxisSpacing) - IntIntPair.m367getFirstimpl(intIntPairM948ellipsisSizeF35zmw$foundation_layout_release.getPackedValue()) < 0) {
                if (isEllipsisWrap) {
                    return new WrapInfo(true, true);
                }
                WrapInfo wrapInfoM943getWrapInfoOpUlnko = m943getWrapInfoOpUlnko(false, 0, IntIntPair.m363constructorimpl(Constraints.m6070getMaxWidthimpl(this.constraints), (IntIntPair.m368getSecondimpl(leftOver) - this.crossAxisSpacing) - Math.max(currentLineCrossAxisSize, IntIntPair.m368getSecondimpl(nextSize.getPackedValue()))), intIntPairM948ellipsisSizeF35zmw$foundation_layout_release, lineIndex + 1, iMax, 0, true, true);
                return new WrapInfo(wrapInfoM943getWrapInfoOpUlnko.getIsLastItemInContainer(), wrapInfoM943getWrapInfoOpUlnko.getIsLastItemInContainer());
            }
        }
