            ...
            while (true) {
                    ...
                    try {
                        ...
                        if (this.readBuffer.size() > j) {
                            ...
                            try {
                                ...
                                long unacknowledgedBytesRead2 = Http2Stream.this.getReadBytesTotal() - Http2Stream.this.getReadBytesAcknowledged();
                                if (errorExceptionToDeliver == null && unacknowledgedBytesRead2 >= Http2Stream.this.getConnection().getOkHttpSettings().getInitialWindowSize() / 2) {
                                    Http2Stream.this.getConnection().writeWindowUpdateLater$okhttp(Http2Stream.this.getId(), unacknowledgedBytesRead2);
                ...
                if (!tryAgain) {
                    if (unacknowledgedBytesRead != -1) {
                        updateConnectionFlowControl(unacknowledgedBytesRead);
                ...
                try {
                    ...
                    try {
                        ...
                        if (outFinishedOnLastFrame) {
                            boolean outFinished = toWrite == this.sendBuffer.size();
                            try {
                                Unit unit = Unit.INSTANCE;
                                Http2Stream.this.getWriteTimeout().enter();
                                try {
