...
class RiskFusionAgent:
    def __init__(self, model_id: str | None = None):
        ...
        self.agent = Agent(
            ...
            description="Android static risk fusion analyst.",
    ...
    def _build_instructions(self) -> list[str]:
        return [
            "You receive manifest reasoning and code reasoning from previous specialized agents.",
            "Fuse both perspectives into a coherent Android static risk assessment.",
            "Prioritize risks supported by both attack surface and code evidence.",
            "Produce likely vulnerability candidates, not just isolated findings.",
    ...
    def build_prompt(self, state: CaseState) -> str:
        ...
        if not state.manifest_reasoning_path or not state.manifest_reasoning_path.exists():
            raise RuntimeError("Manifest reasoning not available")
        if not state.code_reasoning_path or not state.code_reasoning_path.exists():
            raise RuntimeError("Code reasoning not available")

        bundle = json.loads(state.static_analysis_bundle_path.read_text(encoding="utf-8"))
        manifest_reasoning = json.loads(state.manifest_reasoning_path.read_text(encoding="utf-8"))
        code_reasoning = json.loads(state.code_reasoning_path.read_text(encoding="utf-8"))

        fusion_view = {
            "case_id": state.case_id,
            "bundle_summary": bundle.get("summary", {}),
            "manifest_reasoning": manifest_reasoning,
            "code_reasoning": code_reasoning,
        }

        return f"""
FUSED STATIC ANALYSIS VIEW:
{json.dumps(fusion_view, indent=2, ensure_ascii=False)}
"""
