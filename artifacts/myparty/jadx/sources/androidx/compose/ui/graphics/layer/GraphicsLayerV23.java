...
public final class GraphicsLayerV23 implements GraphicsLayerImpl {
    ...
    public GraphicsLayerV23(View view, long j, CanvasHolder canvasHolder, CanvasDrawScope canvasDrawScope) {
        ...
        this.outlineSize = IntSize.INSTANCE.m6292getZeroYbymL2g();
        ...
        renderNodeCreate.setClipToBounds(false);
        m4328applyCompositingStrategyWpw9cng(CompositingStrategy.INSTANCE.m4293getAutoke2Ky5w());
        this.compositingStrategy = CompositingStrategy.INSTANCE.m4293getAutoke2Ky5w();
        this.blendMode = BlendMode.INSTANCE.m3599getSrcOver0nO6VwU();
        this.alpha = 1.0f;
        this.pivotOffset = Offset.INSTANCE.m3431getUnspecifiedF1C5BW0();
        this.scaleX = 1.0f;
        this.scaleY = 1.0f;
        this.ambientShadowColor = Color.INSTANCE.m3683getBlack0d7_KjU();
        this.spotShadowColor = Color.INSTANCE.m3683getBlack0d7_KjU();
    ...
    public void mo4323setCompositingStrategyWpw9cng(int i) {
        this.compositingStrategy = i;
        updateLayerProperties();
    }

    /* renamed from: applyCompositingStrategy-Wpw9cng, reason: not valid java name */
    private final void m4328applyCompositingStrategyWpw9cng(int compositingStrategy) {
        RenderNode renderNode = this.renderNode;
        if (CompositingStrategy.m4289equalsimpl0(compositingStrategy, CompositingStrategy.INSTANCE.m4295getOffscreenke2Ky5w())) {
            renderNode.setLayerType(2);
            renderNode.setLayerPaint(this.layerPaint);
            renderNode.setHasOverlappingRendering(true);
        } else if (CompositingStrategy.m4289equalsimpl0(compositingStrategy, CompositingStrategy.INSTANCE.m4294getModulateAlphake2Ky5w())) {
    ...
    public void mo4322setBlendModes9anfk8(int i) {
        if (BlendMode.m3568equalsimpl0(this.blendMode, i)) {
            return;
        }
        this.blendMode = i;
        obtainLayerPaint().setXfermode(new PorterDuffXfermode(AndroidBlendMode_androidKt.m3509toPorterDuffModes9anfk8(i)));
        updateLayerProperties();
    }

    private final boolean requiresCompositingLayer() {
        return (!CompositingStrategy.m4289equalsimpl0(getCompositingStrategy(), CompositingStrategy.INSTANCE.m4295getOffscreenke2Ky5w()) && BlendMode.m3568equalsimpl0(getBlendMode(), BlendMode.INSTANCE.m3599getSrcOver0nO6VwU()) && getColorFilter() == null) ? false : true;
    }

    private final void updateLayerProperties() {
        if (requiresCompositingLayer()) {
            m4328applyCompositingStrategyWpw9cng(CompositingStrategy.INSTANCE.m4295getOffscreenke2Ky5w());
        } else {
            m4328applyCompositingStrategyWpw9cng(getCompositingStrategy());
        }
    }
    ...
    public void mo4325setPivotOffsetk4lQ0M(long j) {
        this.pivotOffset = j;
        if (OffsetKt.m3437isUnspecifiedk4lQ0M(j)) {
            this.shouldManuallySetCenterPivot = true;
            this.renderNode.setPivotX(IntSize.m6287getWidthimpl(this.size) / 2.0f);
            this.renderNode.setPivotY(IntSize.m6286getHeightimpl(this.size) / 2.0f);
        } else {
            this.shouldManuallySetCenterPivot = false;
            this.renderNode.setPivotX(Offset.m3416getXimpl(j));
            this.renderNode.setPivotY(Offset.m3417getYimpl(j));
        }
    }
    ...
    public void mo4321setAmbientShadowColor8_81llA(long j) {
        if (Build.VERSION.SDK_INT >= 28) {
            this.ambientShadowColor = j;
            RenderNodeVerificationHelper28.INSTANCE.setAmbientShadowColor(this.renderNode, ColorKt.m3711toArgb8_81llA(j));
        }
    }
    ...
    public void mo4327setSpotShadowColor8_81llA(long j) {
        if (Build.VERSION.SDK_INT >= 28) {
            this.spotShadowColor = j;
            RenderNodeVerificationHelper28.INSTANCE.setSpotShadowColor(this.renderNode, ColorKt.m3711toArgb8_81llA(j));
        }
    }
    ...
    public void mo4326setPositionH0pRuoY(int x, int y, long size) {
        this.renderNode.setLeftTopRightBottom(x, y, IntSize.m6287getWidthimpl(size) + x, IntSize.m6286getHeightimpl(size) + y);
        if (IntSize.m6285equalsimpl0(this.size, size)) {
            return;
        }
        if (this.shouldManuallySetCenterPivot) {
            this.renderNode.setPivotX(IntSize.m6287getWidthimpl(size) / 2.0f);
            this.renderNode.setPivotY(IntSize.m6286getHeightimpl(size) / 2.0f);
        }
        this.size = size;
    }
    ...
    public void mo4324setOutlineO0kMr_c(Outline outline, long outlineSize) {
    ...
    public void record(Density density, LayoutDirection layoutDirection, GraphicsLayer layer, Function1<? super DrawScope, Unit> block) {
        Canvas canvasStart = this.renderNode.start(Math.max(IntSize.m6287getWidthimpl(this.size), IntSize.m6287getWidthimpl(this.outlineSize)), Math.max(IntSize.m6286getHeightimpl(this.size), IntSize.m6286getHeightimpl(this.outlineSize)));
        try {
            ...
            drawContext.mo4135setSizeuvyYCjk(jM6299toSizeozmzZPI);
