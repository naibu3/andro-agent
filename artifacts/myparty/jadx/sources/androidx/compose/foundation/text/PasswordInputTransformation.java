...
public final class PasswordInputTransformation implements InputTransformation {
    ...
    public void transformInput(TextFieldBuffer textFieldBuffer) {
        if (textFieldBuffer.getChanges().getChangeCount() != 1 || TextRange.m5598getLengthimpl(textFieldBuffer.getChanges().mo1446getRangejx7JFs(0)) != 1 || TextRange.m5598getLengthimpl(textFieldBuffer.getChanges().mo1445getOriginalRangejx7JFs(0)) != 0 || textFieldBuffer.hasSelection()) {
            setRevealCodepointIndex(-1);
            return;
        }
        int iM5600getMinimpl = TextRange.m5600getMinimpl(textFieldBuffer.getChanges().mo1446getRangejx7JFs(0));
        if (getRevealCodepointIndex$foundation_release() != iM5600getMinimpl) {
            this.scheduleHide.invoke();
            setRevealCodepointIndex(iM5600getMinimpl);
        }
    }
