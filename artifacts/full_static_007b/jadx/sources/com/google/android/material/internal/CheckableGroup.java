...
public class CheckableGroup<T extends MaterialCheckable<T>> {
    ...
    public void check(int id2) {
        T checkable = this.checkables.get(Integer.valueOf(id2));
        if (checkable != null && checkInternal(checkable)) {
            onCheckedStateChanged();
        }
    }

    public void uncheck(int id2) {
        T checkable = this.checkables.get(Integer.valueOf(id2));
    ...
    public boolean checkInternal(MaterialCheckable<T> checkable) {
        ...
        boolean checkedStateChanged = this.checkedIds.add(Integer.valueOf(id2));
    ...
    public boolean uncheckInternal(MaterialCheckable<T> checkable, boolean selectionRequired) {
        ...
        if (selectionRequired && this.checkedIds.size() == 1 && this.checkedIds.contains(Integer.valueOf(id2))) {
            checkable.setChecked(true);
            return false;
        }
        boolean checkedStateChanged = this.checkedIds.remove(Integer.valueOf(id2));
        if (checkable.isChecked()) {
