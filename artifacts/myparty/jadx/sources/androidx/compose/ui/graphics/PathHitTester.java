...
public final class PathHitTester {
    ...
    public final boolean m3941containsk4lQ0M(long position) {
        int i;
        int iLineWinding;
        if (!this.path.isEmpty() && this.bounds.m3442containsk4lQ0M(position)) {
            ...
            if (((IntervalTree) intervalTree).root != ((IntervalTree) intervalTree).terminator) {
                ...
                while (arrayList.size() > 0) {
                    ...
                    if (node.overlaps(fM3417getYimpl, fM3417getYimpl)) {
                        T data = node.getData();
                        ...
                        if (i2 == 1) {
                            iLineWinding = BezierKt.lineWinding(points, fM3416getXimpl, fM3417getYimpl);
                        } else if (i2 == 2) {
                            iLineWinding = BezierKt.quadraticWinding(points, fM3416getXimpl, fM3417getYimpl, fArr, fArr2);
                        } else if (i2 == 3) {
                            iLineWinding = BezierKt.cubicWinding(points, fM3416getXimpl, fM3417getYimpl, fArr, fArr2);
                        }
                        i += iLineWinding;
                    }
                    if (node.getLeft() != ((IntervalTree) intervalTree).terminator && node.getLeft().getMax() >= fM3417getYimpl) {
                        arrayList.add(node.getLeft());
                    }
                    if (node.getRight() != ((IntervalTree) intervalTree).terminator && node.getRight().getMin() <= fM3417getYimpl) {
            ...
            if (PathFillType.m3935equalsimpl0(this.path.mo3548getFillTypeRgk1Os(), PathFillType.INSTANCE.m3939getEvenOddRgk1Os())) {
                i &= 1;
            }
            if (i != 0) {
                return true;
