...
public final class ContentInViewNode extends Modifier.Node implements BringIntoViewResponder, LayoutAwareModifierNode, CompositionLocalConsumerModifierNode {
    ...
    private long viewportSize = IntSize.INSTANCE.m6292getZeroYbymL2g();
    ...
    public Rect calculateRectForParent(Rect localRect) {
        if (IntSize.m6285equalsimpl0(this.viewportSize, IntSize.INSTANCE.m6292getZeroYbymL2g())) {
            throw new IllegalStateException("Expected BringIntoViewRequester to not be used before parents are placed.".toString());
        }
        return m736computeDestinationO0kMr_c(localRect, this.viewportSize);
    }
    ...
    public Object bringChildIntoView(Function0<Rect> function0, Continuation<? super Unit> continuation) {
        ...
        if (rectInvoke == null || m738isMaxVisibleO0kMr_c$default(this, rectInvoke, 0L, 1, null)) {
    ...
    public void mo702onRemeasuredozmzZPI(long size) {
        Rect focusedChildBounds;
        long j = this.viewportSize;
        this.viewportSize = size;
        if (m734compareToTemP2vQ(size, j) < 0 && (focusedChildBounds = getFocusedChildBounds()) != null) {
            Rect rect = this.focusedChildBoundsFromPreviousRemeasure;
            if (rect == null) {
                rect = focusedChildBounds;
            }
            if (!this.isAnimationRunning && !this.trackingFocusedChild && m737isMaxVisibleO0kMr_c(rect, j) && !m737isMaxVisibleO0kMr_c(focusedChildBounds, size)) {
                ...
                if (i == 0) {
                    ...
                    Function1<Float, Unit> function1 = new Function1<Float, Unit>() { // from class: androidx.compose.foundation.gestures.ContentInViewNode.launchAnimation.2.1.1
                        ...
                        public final void invoke(float f) {
                            ...
                            float fM837toFloatk4lQ0M = f2 * scrollingLogic.m837toFloatk4lQ0M(scrollingLogic.m835reverseIfNeededMKHz9U(nestedScrollScope.mo793scrollByOzD1aCk(scrollingLogic.m835reverseIfNeededMKHz9U(scrollingLogic.m838toOffsettuRUvjQ(f2 * f)), NestedScrollSource.INSTANCE.m4759getUserInputWNlRxjI())));
                            if (Math.abs(fM837toFloatk4lQ0M) < Math.abs(f)) {
                                JobKt__JobKt.cancel$default(job, "Scroll animation cancelled because scroll was not consumed (" + fM837toFloatk4lQ0M + " < " + f + ')', null, 2, null);
                            }
                        }
                    };
                    ...
                    if (updatableAnimationState.animateToZero(function1, new Function0<Unit>() { // from class: androidx.compose.foundation.gestures.ContentInViewNode.launchAnimation.2.1.2
                        ...
                        public final void invoke2() {
                            ...
                            while (true) {
                                ...
                                if (!(rectInvoke == null ? true : ContentInViewNode.m738isMaxVisibleO0kMr_c$default(contentInViewNode3, rectInvoke, 0L, 1, null))) {
                                    break;
                                }
                                ...
                                continuation.resumeWith(Result.m9118constructorimpl(unit));
                            }
                            if (contentInViewNode2.trackingFocusedChild) {
                                Rect focusedChildBounds = contentInViewNode2.getFocusedChildBounds();
                                if (focusedChildBounds != null && ContentInViewNode.m738isMaxVisibleO0kMr_c$default(contentInViewNode2, focusedChildBounds, 0L, 1, null)) {
    ...
    public final float calculateScrollDelta(BringIntoViewSpec bringIntoViewSpec) {
        ...
        long jM6299toSizeozmzZPI = IntSizeKt.m6299toSizeozmzZPI(this.viewportSize);
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return bringIntoViewSpec.calculateScrollDistance(rectFindBringIntoViewRequest.getTop(), rectFindBringIntoViewRequest.getBottom() - rectFindBringIntoViewRequest.getTop(), Size.m3482getHeightimpl(jM6299toSizeozmzZPI));
        }
        if (i == 2) {
            return bringIntoViewSpec.calculateScrollDistance(rectFindBringIntoViewRequest.getLeft(), rectFindBringIntoViewRequest.getRight() - rectFindBringIntoViewRequest.getLeft(), Size.m3485getWidthimpl(jM6299toSizeozmzZPI));
        }
    ...
    private final Rect findBringIntoViewRequest() {
        ...
        if (size > 0) {
            ...
            do {
                ...
                if (rectInvoke != null) {
                    if (m735compareToiLBOSCw(rectInvoke.m3449getSizeNHjbRc(), IntSizeKt.m6299toSizeozmzZPI(this.viewportSize)) > 0) {
    ...
    private final Rect m736computeDestinationO0kMr_c(Rect childBounds, long containerSize) {
        return childBounds.m3453translatek4lQ0M(Offset.m3425unaryMinusF1C5BW0(m739relocationOffsetBMxPBkI(childBounds, containerSize)));
    }

    /* renamed from: isMaxVisible-O0kMr_c$default, reason: not valid java name */
    static /* synthetic */ boolean m738isMaxVisibleO0kMr_c$default(ContentInViewNode contentInViewNode, Rect rect, long j, int i, Object obj) {
        if ((i & 1) != 0) {
            j = contentInViewNode.viewportSize;
        }
        return contentInViewNode.m737isMaxVisibleO0kMr_c(rect, j);
    }

    /* renamed from: isMaxVisible-O0kMr_c, reason: not valid java name */
    private final boolean m737isMaxVisibleO0kMr_c(Rect rect, long j) {
        long jM739relocationOffsetBMxPBkI = m739relocationOffsetBMxPBkI(rect, j);
        return Math.abs(Offset.m3416getXimpl(jM739relocationOffsetBMxPBkI)) <= 0.5f && Math.abs(Offset.m3417getYimpl(jM739relocationOffsetBMxPBkI)) <= 0.5f;
    }

    /* renamed from: relocationOffset-BMxPBkI, reason: not valid java name */
    private final long m739relocationOffsetBMxPBkI(Rect childBounds, long containerSize) {
        long jM6299toSizeozmzZPI = IntSizeKt.m6299toSizeozmzZPI(containerSize);
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return OffsetKt.Offset(0.0f, requireBringIntoViewSpec().calculateScrollDistance(childBounds.getTop(), childBounds.getBottom() - childBounds.getTop(), Size.m3482getHeightimpl(jM6299toSizeozmzZPI)));
        }
        if (i == 2) {
            return OffsetKt.Offset(requireBringIntoViewSpec().calculateScrollDistance(childBounds.getLeft(), childBounds.getRight() - childBounds.getLeft(), Size.m3485getWidthimpl(jM6299toSizeozmzZPI)), 0.0f);
        }
        throw new NoWhenBranchMatchedException();
    }

    /* renamed from: compareTo-TemP2vQ, reason: not valid java name */
    private final int m734compareToTemP2vQ(long j, long j2) {
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return Intrinsics.compare(IntSize.m6286getHeightimpl(j), IntSize.m6286getHeightimpl(j2));
        }
        if (i == 2) {
            return Intrinsics.compare(IntSize.m6287getWidthimpl(j), IntSize.m6287getWidthimpl(j2));
        }
        throw new NoWhenBranchMatchedException();
    }

    /* renamed from: compareTo-iLBOSCw, reason: not valid java name */
    private final int m735compareToiLBOSCw(long j, long j2) {
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return Float.compare(Size.m3482getHeightimpl(j), Size.m3482getHeightimpl(j2));
        }
        if (i == 2) {
            return Float.compare(Size.m3485getWidthimpl(j), Size.m3485getWidthimpl(j2));
        }
        throw new NoWhenBranchMatchedException();
    }
