        ...
        switch (event.getAction()) {
            ...
            case 1:
                ...
                if (this.mRotationCenterId != -1) {
                    ...
                    rcx2 = this.mTempLoc[0] + ((v.getLeft() + v.getRight()) / 2.0f);
                    rcy2 = this.mTempLoc[1] + ((v.getTop() + v.getBottom()) / 2.0f);
                } else {
                    if (this.mTouchAnchorId != -1) {
                        ...
                        this.mMotionLayout.getLocationOnScreen(this.mTempLoc);
                        float rcx3 = this.mTempLoc[0] + ((v2.getLeft() + v2.getRight()) / 2.0f);
                        float rcy3 = this.mTempLoc[1] + ((v2.getTop() + v2.getBottom()) / 2.0f);
                        rcx = rcx3;
                        rcy = rcy3;
                    }
                    float rcx4 = event.getRawX();
                    float relativePosX = rcx4 - rcx;
                    ...
                    angularVelocity = ((float) (angle2 - angle1)) * 62.5f;
                    if (!Float.isNaN(angularVelocity)) {
                        pos2 += ((angularVelocity * 3.0f) * this.mDragScale) / this.mAnchorDpDt[1];
                    }
                    if (pos2 == 0.0f && pos2 != 1.0f && this.mOnTouchUp != 3) {
                        float angularVelocity4 = (this.mDragScale * angularVelocity) / this.mAnchorDpDt[1];
                        float target = ((double) pos2) < 0.5d ? 0.0f : 1.0f;
                        if (this.mOnTouchUp != 6) {
                            angularVelocity2 = angularVelocity4;
                        } else {
                            if (currentPos + angularVelocity4 >= 0.0f) {
                                angularVelocity2 = angularVelocity4;
                            } else {
                                angularVelocity2 = Math.abs(angularVelocity4);
                            }
                            target = 1.0f;
                        }
                        float angularVelocity5 = angularVelocity2;
                        if (this.mOnTouchUp != 7) {
                            angularVelocity3 = angularVelocity5;
                        } else {
                            if (currentPos + angularVelocity5 <= 1.0f) {
                                angularVelocity3 = angularVelocity5;
                            } else {
                                angularVelocity3 = -Math.abs(angularVelocity5);
                            }
                            target = 0.0f;
                        }
                        float tvx2 = angularVelocity3 * 3.0f;
                        this.mMotionLayout.touchAnimateTo(this.mOnTouchUp, target, tvx2);
                ...
                rcx = rcx2;
                rcy = rcy2;
                float rcx42 = event.getRawX();
                float relativePosX2 = rcx42 - rcx;
                float relativePosY2 = event.getRawY() - rcy;
                double angle12 = Math.toDegrees(Math.atan2(relativePosY2, relativePosX2));
                if (this.mTouchAnchorId == -1) {
                }
                double angle22 = Math.toDegrees(Math.atan2(tvy + relativePosY2, tvx + relativePosX2));
                angularVelocity = ((float) (angle22 - angle12)) * 62.5f;
                if (!Float.isNaN(angularVelocity)) {
                }
                if (pos2 == 0.0f) {
                }
            ...
            case 2:
                ...
                float rcx5 = this.mMotionLayout.getWidth() / 2.0f;
                float rcy4 = this.mMotionLayout.getHeight() / 2.0f;
                if (this.mRotationCenterId != -1) {
                    View v3 = this.mMotionLayout.findViewById(this.mRotationCenterId);
                    f = 360.0f;
                    this.mMotionLayout.getLocationOnScreen(this.mTempLoc);
                    rcx5 = this.mTempLoc[0] + ((v3.getLeft() + v3.getRight()) / 2.0f);
                    rcy4 = this.mTempLoc[1] + ((v3.getTop() + v3.getBottom()) / 2.0f);
                } else {
                    f = 360.0f;
                    if (this.mTouchAnchorId != -1) {
                        ...
                        if (v4 != null) {
                            this.mMotionLayout.getLocationOnScreen(this.mTempLoc);
                            rcx5 = this.mTempLoc[0] + ((v4.getLeft() + v4.getRight()) / 2.0f);
                            rcy4 = this.mTempLoc[1] + ((v4.getTop() + v4.getBottom()) / 2.0f);
                        } else {
                            Log.e(TAG, "could not find view to animate to");
                        }
                    }
                }
                float relativePosX3 = event.getRawX() - rcx5;
                float relativePosY3 = event.getRawY() - rcy4;
                double angle13 = Math.atan2(event.getRawY() - rcy4, event.getRawX() - rcx5);
                double angle23 = Math.atan2(this.mLastTouchY - rcy4, this.mLastTouchX - rcx5);
                float drag = (float) (((angle13 - angle23) * 180.0d) / 3.141592653589793d);
                if (drag > 330.0f) {
                    drag -= f;
                } else if (drag < -330.0f) {
                    drag += f;
                ...
                if (Math.abs(drag) > 0.01d || this.mDragStarted) {
                    ...
                    if (pos4 == current) {
                        ...
                        double angle24 = tvx3;
                        this.mMotionLayout.mLastVelocity = (float) Math.toDegrees((float) ((Math.sin(Math.atan2(tvy2, tvx3) - angle13) * Math.hypot(tvy2, angle24)) / Math.hypot(relativePosX3, relativePosY3)));
                    }
                    this.mLastTouchX = event.getRawX();
                    this.mLastTouchY = event.getRawY();
                    break;
