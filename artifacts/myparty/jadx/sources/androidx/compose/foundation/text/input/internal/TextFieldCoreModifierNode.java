...
public final class TextFieldCoreModifierNode extends DelegatingNode implements LayoutModifierNode, DrawModifierNode, CompositionLocalConsumerModifierNode, GlobalPositionAwareModifierNode, SemanticsModifierNode {
    ...
    public MeasureResult mo430measure3p2s80s(MeasureScope measureScope, Measurable measurable, long j) {
        if (this.orientation == Orientation.Vertical) {
            return m1538measureVerticalScroll3p2s80s(measureScope, measurable, j);
        }
        return m1537measureHorizontalScroll3p2s80s(measureScope, measurable, j);
    }
    ...
    public void draw(ContentDrawScope contentDrawScope) {
        ...
        if (TextRange.m5596getCollapsedimpl(visualText.getSelection())) {
            ...
            if (visualText.shouldShowSelection()) {
                m1536drawSelectionSbBc2M(contentDrawScope, visualText.getSelection(), layoutResult);
            }
    ...
    private final MeasureResult m1538measureVerticalScroll3p2s80s(final MeasureScope measureScope, Measurable measurable, long j) {
        final Placeable placeableMo4993measureBRTryo0 = measurable.mo4993measureBRTryo0(Constraints.m6061copyZbe2FdA$default(j, 0, 0, 0, Integer.MAX_VALUE, 7, null));
        final int iMin = Math.min(placeableMo4993measureBRTryo0.getHeight(), Constraints.m6069getMaxHeightimpl(j));
        return MeasureScope.layout$default(measureScope, placeableMo4993measureBRTryo0.getWidth(), iMin, null, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.text.input.internal.TextFieldCoreModifierNode$measureVerticalScroll$1
            ...
            public final void invoke2(Placeable.PlacementScope placementScope) {
                this.this$0.m1539updateScrollStatetIlFzwE(measureScope, iMin, placeableMo4993measureBRTryo0.getHeight(), this.this$0.textFieldState.getVisualText().getSelection(), measureScope.getLayoutDirection());
                Placeable.PlacementScope.placeRelative$default(placementScope, placeableMo4993measureBRTryo0, 0, -this.this$0.scrollState.getValue(), 0.0f, 4, null);
            }
        }, 4, null);
    }

    /* renamed from: measureHorizontalScroll-3p2s80s, reason: not valid java name */
    private final MeasureResult m1537measureHorizontalScroll3p2s80s(final MeasureScope measureScope, Measurable measurable, long j) {
        final Placeable placeableMo4993measureBRTryo0 = measurable.mo4993measureBRTryo0(Constraints.m6061copyZbe2FdA$default(j, 0, Integer.MAX_VALUE, 0, 0, 13, null));
        final int iMin = Math.min(placeableMo4993measureBRTryo0.getWidth(), Constraints.m6070getMaxWidthimpl(j));
        return MeasureScope.layout$default(measureScope, iMin, placeableMo4993measureBRTryo0.getHeight(), null, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.text.input.internal.TextFieldCoreModifierNode$measureHorizontalScroll$1
            /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
            {
                super(1);
            }
            ...
            /* renamed from: invoke, reason: avoid collision after fix types in other method */
            public final void invoke2(Placeable.PlacementScope placementScope) {
                this.this$0.m1539updateScrollStatetIlFzwE(measureScope, iMin, placeableMo4993measureBRTryo0.getWidth(), this.this$0.textFieldState.getVisualText().getSelection(), measureScope.getLayoutDirection());
                Placeable.PlacementScope.placeRelative$default(placementScope, placeableMo4993measureBRTryo0, -this.this$0.scrollState.getValue(), 0, 0.0f, 4, null);
            }
        }, 4, null);
    }

    /* renamed from: calculateOffsetToFollow-72CqOWE, reason: not valid java name */
    private final int m1535calculateOffsetToFollow72CqOWE(long currSelection, int currTextLayoutSize) {
        TextRange textRange = this.previousSelection;
        if (textRange == null || TextRange.m5597getEndimpl(currSelection) != TextRange.m5597getEndimpl(textRange.getPackedValue())) {
            return TextRange.m5597getEndimpl(currSelection);
        }
        TextRange textRange2 = this.previousSelection;
        if (textRange2 == null || TextRange.m5602getStartimpl(currSelection) != TextRange.m5602getStartimpl(textRange2.getPackedValue())) {
            return TextRange.m5602getStartimpl(currSelection);
        }
    ...
    */
    public final void m1539updateScrollStatetIlFzwE(Density density, int i, int i2, long j, LayoutDirection layoutDirection) {
        TextLayoutResult layoutResult;
        float f;
        this.scrollState.setMaxValue$foundation_release(i2 - i);
        int iM1535calculateOffsetToFollow72CqOWE = m1535calculateOffsetToFollow72CqOWE(j, i2);
        if (iM1535calculateOffsetToFollow72CqOWE < 0 || !getShowCursor() || (layoutResult = this.textLayoutState.getLayoutResult()) == null) {
            return;
        }
        Rect cursorRect = layoutResult.getCursorRect(RangesKt.coerceIn(iM1535calculateOffsetToFollow72CqOWE, (ClosedRange<Integer>) new IntRange(0, layoutResult.getLayoutInput().getText().length())));
        ...
        this.previousSelection = TextRange.m5590boximpl(j);
    ...
    private final void m1536drawSelectionSbBc2M(DrawScope drawScope, long j, TextLayoutResult textLayoutResult) {
        ...
        if (iM5600getMinimpl != iM5599getMaximpl) {
            DrawScope.m4203drawPathLG529CI$default(drawScope, textLayoutResult.getPathForRange(iM5600getMinimpl, iM5599getMaximpl), ((TextSelectionColors) CompositionLocalConsumerModifierNodeKt.currentValueOf(this, TextSelectionColorsKt.getLocalTextSelectionColors())).getBackgroundColor(), 0.0f, null, null, 0, 60, null);
        }
    }
    ...
    private final void drawHighlight(DrawScope drawScope, Pair<TextHighlightType, TextRange> pair, TextLayoutResult textLayoutResult) {
        ...
        if (TextRange.m5596getCollapsedimpl(packedValue)) {
            return;
        }
        Path pathForRange = textLayoutResult.getPathForRange(TextRange.m5600getMinimpl(packedValue), TextRange.m5599getMaximpl(packedValue));
        if (TextHighlightType.m1456equalsimpl0(value, TextHighlightType.INSTANCE.m1460getHandwritingDeletePreviewsxJuwY())) {
            Brush brush = textLayoutResult.getLayoutInput().getStyle().getBrush();
            if (brush != null) {
                DrawScope.m4202drawPathGBMwjPU$default(drawScope, pathForRange, brush, 0.2f, null, null, 0, 56, null);
                return;
            }
            long jM5632getColor0d7_KjU = textLayoutResult.getLayoutInput().getStyle().m5632getColor0d7_KjU();
            if (jM5632getColor0d7_KjU == 16) {
                jM5632getColor0d7_KjU = Color.INSTANCE.m3683getBlack0d7_KjU();
            }
            long j = jM5632getColor0d7_KjU;
            DrawScope.m4203drawPathLG529CI$default(drawScope, pathForRange, Color.m3656copywmQWz5c$default(j, Color.m3659getAlphaimpl(j) * 0.2f, 0.0f, 0.0f, 0.0f, 14, null), 0.0f, null, null, 0, 60, null);
            return;
        }
        DrawScope.m4203drawPathLG529CI$default(drawScope, pathForRange, ((TextSelectionColors) CompositionLocalConsumerModifierNodeKt.currentValueOf(this, TextSelectionColorsKt.getLocalTextSelectionColors())).getBackgroundColor(), 0.0f, null, null, 0, 60, null);
    }
    ...
    private final void drawCursor(DrawScope drawScope) {
        ...
        if (cursorAlpha != 0.0f && getShowCursor()) {
            ...
            DrawScope.m4198drawLine1RTmtNc$default(drawScope, this.cursorBrush, cursorRect.m3450getTopCenterF1C5BW0(), cursorRect.m3443getBottomCenterF1C5BW0(), cursorRect.getWidth(), 0, null, cursorAlpha, null, 0, 432, null);
        }
    }
