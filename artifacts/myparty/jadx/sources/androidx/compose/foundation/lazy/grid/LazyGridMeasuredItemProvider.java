...
public abstract class LazyGridMeasuredItemProvider implements LazyLayoutMeasuredItemProvider<LazyGridMeasuredItem> {
    ...
    public abstract LazyGridMeasuredItem mo1159createItemO3s9Psw(int index, Object key, Object contentType, int crossAxisSize, int mainAxisSpacing, List<? extends Placeable> placeables, long constraints, int lane, int span);
    ...
    public LazyGridMeasuredItem mo1134getAndMeasurehBUhpc(int index, int lane, int span, long constraints) {
        return m1164getAndMeasurem8Kt_7k(index, constraints, lane, span, this.defaultMainAxisSpacing);
    }

    /* renamed from: getAndMeasure-m8Kt_7k, reason: not valid java name */
    public final LazyGridMeasuredItem m1164getAndMeasurem8Kt_7k(int index, long constraints, int lane, int span, int mainAxisSpacing) {
        ...
        List<Placeable> listMo1191measure0kLqBqw = this.measureScope.mo1191measure0kLqBqw(index, constraints);
        if (Constraints.m6068getHasFixedWidthimpl(constraints)) {
            iM6071getMinHeightimpl = Constraints.m6072getMinWidthimpl(constraints);
        } else {
            if (!Constraints.m6067getHasFixedHeightimpl(constraints)) {
                throw new IllegalArgumentException("does not have fixed height".toString());
            }
            iM6071getMinHeightimpl = Constraints.m6071getMinHeightimpl(constraints);
        }
        return mo1159createItemO3s9Psw(index, key, contentType, iM6071getMinHeightimpl, mainAxisSpacing, listMo1191measure0kLqBqw, constraints, lane, span);
    }
