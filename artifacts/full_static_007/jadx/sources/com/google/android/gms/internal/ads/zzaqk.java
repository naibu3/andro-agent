...
public final class zzaqk {
    public static X509Certificate[][] zza(String str) throws IOException, zzaqh, SecurityException {
        RandomAccessFile randomAccessFile = new RandomAccessFile(str, "r");
        try {
            Pair pairZzc = zzaql.zzc(randomAccessFile);
            if (pairZzc == null) {
            ...
            long jZza = zzaql.zza(byteBuffer);
            if (jZza >= jLongValue) {
                throw new zzaqh("ZIP Central Directory offset out of range: " + jZza + ". ZIP End of Central Directory offset: " + jLongValue);
            }
            if (zzaql.zzb(byteBuffer) + jZza != jLongValue) {
                throw new zzaqh("ZIP Central Directory is not immediately followed by End of Central Directory");
            }
            if (jZza < 32) {
                throw new zzaqh("APK too small for APK Signing Block. ZIP Central Directory offset: " + jZza);
            }
            ...
            try {
                ...
                while (byteBufferSlice.hasRemaining()) {
                    ...
                    if (i3 > byteBufferSlice.remaining()) {
                        throw new zzaqh("APK Signing Block entry #" + i2 + " size out of range: " + i3 + ", available: " + byteBufferSlice.remaining());
                    }
                    if (byteBufferSlice.getInt() == 1896449818) {
                        X509Certificate[][] x509CertificateArrZzl = zzl(randomAccessFile.getChannel(), new zzaqg(zze(byteBufferSlice, i3 - 4), jLongValue2, jZza, j2, byteBuffer, null));
    ...
    private static void zzh(Map map, FileChannel fileChannel, long j, long j2, long j3, ByteBuffer byteBuffer) throws SecurityException {
        ...
        zzaqe zzaqeVar = new zzaqe(fileChannel, 0L, j);
        zzaqe zzaqeVar2 = new zzaqe(fileChannel, j2, j3 - j2);
        ByteBuffer byteBufferDuplicate = byteBuffer.duplicate();
        byteBufferDuplicate.order(ByteOrder.LITTLE_ENDIAN);
        zzaql.zzd(byteBufferDuplicate, j);
        zzaqc zzaqcVar = new zzaqc(byteBufferDuplicate);
        int size = map.size();
        ...
        try {
            byte[][] bArrZzk = zzk(iArr, new zzaqd[]{zzaqeVar, zzaqeVar2, zzaqcVar});
    ...
    private static X509Certificate[] zzj(ByteBuffer byteBuffer, Map map, CertificateFactory certificateFactory) throws InvalidKeySpecException, NoSuchAlgorithmException, SignatureException, IOException, InvalidKeyException, SecurityException, InvalidAlgorithmParameterException {
        ...
        try {
            ...
            while (byteBufferZzf6.hasRemaining()) {
                ...
                try {
                    arrayList3.add(new zzaqi((X509Certificate) certificateFactory.generateCertificate(new ByteArrayInputStream(bArrZzi4)), bArrZzi4));
    ...
    private static byte[][] zzk(int[] iArr, zzaqd[] zzaqdVarArr) throws DigestException {
        ...
        while (true) {
            ...
            jZza += (zzaqdVarArr[i3].zza() + 1048575) / PlaybackStateCompat.ACTION_SET_CAPTIONING_ENABLED;
            i3++;
        }
        if (jZza >= 2097151) {
        ...
        int i8 = 0;
        for (i = 3; i7 < i; i = 3) {
            zzaqd zzaqdVar = zzaqdVarArr[i7];
            long j3 = j2;
            long jZza2 = zzaqdVar.zza();
            while (jZza2 > j2) {
                ...
                long j4 = j3;
                try {
                    zzaqdVar.zzb(messageDigestArr, j4, iMin);
                    byte[] bArr4 = bArr3;
                    int i10 = 0;
    ...
    private static X509Certificate[][] zzl(FileChannel fileChannel, zzaqg zzaqgVar) throws SecurityException, CertificateException {
        HashMap map = new HashMap();
        ArrayList arrayList = new ArrayList();
        try {
            CertificateFactory certificateFactory = CertificateFactory.getInstance("X.509");
            try {
                ByteBuffer byteBufferZzf = zzf(zzaqgVar.zza);
                int i = 0;
                ...
                zzh(map, fileChannel, zzaqgVar.zzb, zzaqgVar.zzc, zzaqgVar.zzd, zzaqgVar.zze);
