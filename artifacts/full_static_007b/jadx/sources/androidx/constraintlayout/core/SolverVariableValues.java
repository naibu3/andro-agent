...
public class SolverVariableValues implements ArrayRow.ArrayRowVariables {
    ...
    public int indexOf(SolverVariable variable) {
        ...
        int id2 = variable.f4id;
        int key = this.keys[id2 % this.HASH_SIZE];
        if (key == -1) {
            return -1;
        }
        if (this.variables[key] == id2) {
            return key;
        }
        while (true) {
            int[] iArr = this.nextKeys;
            i = iArr[key];
            if (i == -1 || this.variables[i] == id2) {
                break;
            }
            key = iArr[key];
        }
    ...
    private void addToHashMap(SolverVariable variable, int index) {
        int[] iArr;
        int hash = variable.f4id % this.HASH_SIZE;
        int[] iArr2 = this.keys;
        int key = iArr2[hash];
        if (key == -1) {
            iArr2[hash] = index;
    ...
    private void removeFromHashMap(SolverVariable variable) {
        ...
        if (currentKey != -1 && this.variables[currentKey] == id2) {
    ...
    private void addVariable(int index, SolverVariable variable, float value) {
        this.variables[index] = variable.f4id;
    ...
    public void put(SolverVariable variable, float value) {
        ...
        int j = this.head;
        for (int i = 0; i < count; i++) {
            if (this.variables[j] == variable.f4id) {
                this.values[j] = value;
                return;
            }
            if (this.variables[j] < variable.f4id) {
