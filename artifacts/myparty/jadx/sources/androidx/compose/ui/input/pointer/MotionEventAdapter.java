...
public final class MotionEventAdapter {
    ...
    private final long m4794getComposePointerId_I2yYro(int motionEventPointerId) {
        long jValueAt;
        ...
        return PointerId.m4844constructorimpl(jValueAt);
    }
    ...
    private final PointerInputEventData createPointerInputEventData(PositionCalculator positionCalculator, MotionEvent motionEvent, int index, boolean pressed) {
        long jMo4946localToScreenMKHz9U;
        int iM4944getUnknownT8wyACA;
        long jM3432getZeroF1C5BW0;
        long jM4794getComposePointerId_I2yYro = m4794getComposePointerId_I2yYro(motionEvent.getPointerId(index));
        float pressure = motionEvent.getPressure(index);
        long jOffset = OffsetKt.Offset(motionEvent.getX(index), motionEvent.getY(index));
        long jM3410copydBAh8RU$default = Offset.m3410copydBAh8RU$default(jOffset, 0.0f, 0.0f, 3, null);
        if (index == 0) {
            jMo4946localToScreenMKHz9U = OffsetKt.Offset(motionEvent.getRawX(), motionEvent.getRawY());
            jOffset = positionCalculator.mo4947screenToLocalMKHz9U(jMo4946localToScreenMKHz9U);
        } else if (Build.VERSION.SDK_INT >= 29) {
            jMo4946localToScreenMKHz9U = MotionEventHelper.INSTANCE.m4795toRawOffsetdBAh8RU(motionEvent, index);
            jOffset = positionCalculator.mo4947screenToLocalMKHz9U(jMo4946localToScreenMKHz9U);
        } else {
            jMo4946localToScreenMKHz9U = positionCalculator.mo4946localToScreenMKHz9U(jOffset);
        }
        long j = jMo4946localToScreenMKHz9U;
        long j2 = jOffset;
        int toolType = motionEvent.getToolType(index);
        if (toolType == 0) {
            iM4944getUnknownT8wyACA = PointerType.INSTANCE.m4944getUnknownT8wyACA();
        } else if (toolType == 1) {
            iM4944getUnknownT8wyACA = PointerType.INSTANCE.m4943getTouchT8wyACA();
        } else if (toolType == 2) {
            iM4944getUnknownT8wyACA = PointerType.INSTANCE.m4942getStylusT8wyACA();
        } else if (toolType == 3) {
            iM4944getUnknownT8wyACA = PointerType.INSTANCE.m4941getMouseT8wyACA();
        } else if (toolType == 4) {
            iM4944getUnknownT8wyACA = PointerType.INSTANCE.m4940getEraserT8wyACA();
        } else {
            iM4944getUnknownT8wyACA = PointerType.INSTANCE.m4944getUnknownT8wyACA();
        }
        int i = iM4944getUnknownT8wyACA;
        ...
        if (motionEvent.getActionMasked() == 8) {
            jM3432getZeroF1C5BW0 = OffsetKt.Offset(motionEvent.getAxisValue(10), (-motionEvent.getAxisValue(9)) + 0.0f);
        } else {
            jM3432getZeroF1C5BW0 = Offset.INSTANCE.m3432getZeroF1C5BW0();
        }
        return new PointerInputEventData(jM4794getComposePointerId_I2yYro, motionEvent.getEventTime(), j, j2, pressed, pressure, i, this.activeHoverIds.get(motionEvent.getPointerId(index), false), arrayList, jM3432getZeroF1C5BW0, jM3410copydBAh8RU$default, null);
    }
}
