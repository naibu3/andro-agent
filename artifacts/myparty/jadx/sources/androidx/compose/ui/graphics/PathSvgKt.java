...
public final class PathSvgKt {
    ...
    public static final String toSvg(Path path, boolean z) {
        ...
        if (z) {
            ...
            StringBuilder sbAppend = sb.append("viewBox=\"" + bounds.getLeft() + CardNumberConfig.SEPARATOR + bounds.getTop() + CardNumberConfig.SEPARATOR + bounds.getWidth() + CardNumberConfig.SEPARATOR + bounds.getHeight() + "\">");
        ...
        if (it.hasNext()) {
            if (z) {
                if (PathFillType.m3935equalsimpl0(path.mo3548getFillTypeRgk1Os(), PathFillType.INSTANCE.m3939getEvenOddRgk1Os())) {
            ...
            while (it.hasNext()) {
                ...
                switch (WhenMappings.$EnumSwitchMapping$0[typeNext$default.ordinal()]) {
                    case 1:
                        sb.append(command(PathSegment.Type.Move, type) + fArr[0] + CardNumberConfig.SEPARATOR + fArr[1]);
                        type = typeNext$default;
                        break;
                    case 2:
                        sb.append(command(PathSegment.Type.Line, type) + fArr[2] + CardNumberConfig.SEPARATOR + fArr[3]);
                        type = typeNext$default;
                        break;
                    case 3:
                        sb.append(command(PathSegment.Type.Quadratic, type));
                        sb.append(new StringBuilder().append(fArr[2]).append(CardNumberConfig.SEPARATOR).append(fArr[3]).append(CardNumberConfig.SEPARATOR).append(fArr[4]).append(CardNumberConfig.SEPARATOR).append(fArr[5]).toString());
                        type = typeNext$default;
                        break;
                    case 4:
                    case 7:
                    ...
                    case 5:
                        ...
                        sb.append(new StringBuilder().append(fArr[2]).append(CardNumberConfig.SEPARATOR).append(fArr[3]).append(CardNumberConfig.SEPARATOR).toString());
                        sb.append(new StringBuilder().append(fArr[4]).append(CardNumberConfig.SEPARATOR).append(fArr[5]).append(CardNumberConfig.SEPARATOR).toString());
                        sb.append(new StringBuilder().append(fArr[6]).append(CardNumberConfig.SEPARATOR).append(fArr[7]).toString());
                        type = typeNext$default;
                        break;
                    case 6:
