        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(simpleTypeMarkerPop)) {
                ...
                if (none == null) {
                    ...
                    while (it.hasNext()) {
                        SimpleTypeMarker simpleTypeMarkerMo1498transformType = none.mo1498transformType(typeCheckerState, it.next());
                        if ((typeSystemContext.isClassType(simpleTypeMarkerMo1498transformType) && !typeSystemContext.isMarkedNullable(simpleTypeMarkerMo1498transformType)) || typeSystemContext.isDefinitelyNotNullType(simpleTypeMarkerMo1498transformType)) {
        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(simpleTypeMarkerPop)) {
                ...
                if (supertypesPolicy == null) {
                    ...
                    Iterator<KotlinTypeMarker> it = typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(simpleTypeMarkerPop)).iterator();
                    while (it.hasNext()) {
                        SimpleTypeMarker simpleTypeMarkerMo1498transformType = supertypesPolicy.mo1498transformType(state, it.next());
                        if (INSTANCE.isApplicableAsEndNode(state, simpleTypeMarkerMo1498transformType, end)) {
                            state.clear();
                            return true;
                        }
                        supertypesDeque.add(simpleTypeMarkerMo1498transformType);
