    ...
    private static TypedBundle parse(String str, Ids table, DataType dtype) {
        ...
        try {
            ...
            for (int i = 0; i < n; i++) {
                ...
                int id2 = table.get(type);
                if (id2 == -1) {
                    System.err.println("unknown type " + type);
                } else {
                    switch (dtype.get(id2)) {
                        case 1:
                            bundle.add(id2, parsedContent.getBoolean(i));
                            break;
                        case 2:
                            bundle.add(id2, value.getInt());
                            System.out.println("parse " + type + " INT_MASK > " + value.getInt());
                            break;
                        case 4:
                            bundle.add(id2, value.getFloat());
                            System.out.println("parse " + type + " FLOAT_MASK > " + value.getFloat());
                            break;
                        case 8:
                            bundle.add(id2, value.content());
