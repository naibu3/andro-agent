...
final class DefaultPagerNestedScrollConnection implements NestedScrollConnection {
    ...
    public final long m1252consumeOnOrientationQWom1Mo(long j, Orientation orientation) {
        if (orientation == Orientation.Vertical) {
            return Velocity.m6350copyOhffZ5M$default(j, 0.0f, 0.0f, 2, null);
        }
        return Velocity.m6350copyOhffZ5M$default(j, 0.0f, 0.0f, 1, null);
    }

    @Override // androidx.compose.ui.input.nestedscroll.NestedScrollConnection
    /* renamed from: onPreScroll-OzD1aCk */
    public long mo1096onPreScrollOzD1aCk(long available, int source) {
        if (NestedScrollSource.m4747equalsimpl0(source, NestedScrollSource.INSTANCE.m4759getUserInputWNlRxjI()) && Math.abs(this.state.getCurrentPageOffsetFraction()) > 1.0E-6d) {
            ...
            float fM3417getYimpl = -this.state.dispatchRawDelta(-RangesKt.coerceIn(this.orientation == Orientation.Horizontal ? Offset.m3416getXimpl(available) : Offset.m3417getYimpl(available), currentPageOffsetFraction, pageSize));
            float fM3416getXimpl = this.orientation == Orientation.Horizontal ? fM3417getYimpl : Offset.m3416getXimpl(available);
            if (this.orientation != Orientation.Vertical) {
                fM3417getYimpl = Offset.m3417getYimpl(available);
            }
            return Offset.m3409copydBAh8RU(available, fM3416getXimpl, fM3417getYimpl);
        }
    ...
    public long mo822onPostScrollDzOQY0M(long consumed, long available, int source) {
        if (NestedScrollSource.m4747equalsimpl0(source, NestedScrollSource.INSTANCE.m4758getSideEffectWNlRxjI()) && m1251mainAxisk4lQ0M(available) != 0.0f) {
    ...
    public Object mo821onPostFlingRZ2iAVY(long j, long j2, Continuation<? super Velocity> continuation) {
        return Velocity.m6345boximpl(m1252consumeOnOrientationQWom1Mo(j2, this.orientation));
    }

    /* renamed from: mainAxis-k-4lQ0M, reason: not valid java name */
    private final float m1251mainAxisk4lQ0M(long j) {
