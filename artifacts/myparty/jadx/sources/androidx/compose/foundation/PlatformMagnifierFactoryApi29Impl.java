...
public final class PlatformMagnifierFactoryApi29Impl implements PlatformMagnifierFactory {
    ...
    public PlatformMagnifierImpl mo666createnHHXs2Y(View view, boolean useTextDefault, long size, float cornerRadius, float elevation, boolean clippingEnabled, Density density, float initialZoom) {
        if (useTextDefault) {
            return new PlatformMagnifierImpl(new Magnifier(view));
        }
        long jMo713toSizeXkaWNTQ = density.mo713toSizeXkaWNTQ(size);
        float fMo712toPx0680j_4 = density.mo712toPx0680j_4(cornerRadius);
        float fMo712toPx0680j_42 = density.mo712toPx0680j_4(elevation);
        Magnifier.Builder builder = new Magnifier.Builder(view);
        if (jMo713toSizeXkaWNTQ != InlineClassHelperKt.UnspecifiedPackedFloats) {
            builder.setSize(MathKt.roundToInt(Size.m3485getWidthimpl(jMo713toSizeXkaWNTQ)), MathKt.roundToInt(Size.m3482getHeightimpl(jMo713toSizeXkaWNTQ)));
        }
        if (!Float.isNaN(fMo712toPx0680j_4)) {
            builder.setCornerRadius(fMo712toPx0680j_4);
        }
        if (!Float.isNaN(fMo712toPx0680j_42)) {
            builder.setElevation(fMo712toPx0680j_42);
        }
    ...
    public static final class PlatformMagnifierImpl extends PlatformMagnifierFactoryApi28Impl.PlatformMagnifierImpl {
        ...
        public void mo665updateWko1d7g(long sourceCenter, long magnifierCenter, float zoom) {
            if (!Float.isNaN(zoom)) {
                getMagnifier().setZoom(zoom);
            }
            if (OffsetKt.m3435isSpecifiedk4lQ0M(magnifierCenter)) {
                getMagnifier().show(Offset.m3416getXimpl(sourceCenter), Offset.m3417getYimpl(sourceCenter), Offset.m3416getXimpl(magnifierCenter), Offset.m3417getYimpl(magnifierCenter));
            } else {
                getMagnifier().show(Offset.m3416getXimpl(sourceCenter), Offset.m3417getYimpl(sourceCenter));
            }
        }
    }
}
