    ...
    void adjustCompleteIndicatorColumnRowNumbers(BarcodeMetadata barcodeMetadata) {
        ...
        while (codewordsRow < lastRow) {
            if (codewords[codewordsRow] != null) {
                Codeword codeword2 = codewords[codewordsRow];
                int rowDifference = codeword2.getRowNumber() - barcodeRow;
                if (rowDifference != 0) {
                    if (rowDifference == 1) {
                        ...
                        if (rowDifference >= 0) {
                            codeword = null;
                            if (codeword2.getRowNumber() < barcodeMetadata.getRowCount() && rowDifference <= codewordsRow) {
                                if (maxRowHeight > 2) {
