        ...
        public final void measure(ConstraintWidget widget, BasicMeasure.Measure measure) {
            ...
            switch (horizontalBehavior) {
                case FIXED:
                    ...
                    break;
                case WRAP_CONTENT:
                    horizontalSpec4 = ViewGroup.getChildMeasureSpec(this.layoutWidthSpec, widthPadding, -2);
                    break;
                case MATCH_PARENT:
                    horizontalSpec4 = ViewGroup.getChildMeasureSpec(this.layoutWidthSpec, widget.getHorizontalMargin() + widthPadding, -1);
                    break;
                case MATCH_CONSTRAINT:
                    ...
                    if (measure.measureStrategy == BasicMeasure.Measure.TRY_GIVEN_DIMENSIONS || measure.measureStrategy == BasicMeasure.Measure.USE_GIVEN_DIMENSIONS) {
                        ...
                        if (!useCurrent) {
                            ...
                            horizontalSpec4 = View.MeasureSpec.makeMeasureSpec(widget.getWidth(), BasicMeasure.EXACTLY);
                            break;
                        }
                    }
                    break;
            ...
            switch (verticalBehavior) {
                case FIXED:
                    verticalSpec = View.MeasureSpec.makeMeasureSpec(verticalDimension2, BasicMeasure.EXACTLY);
                    break;
                case WRAP_CONTENT:
            ...
            if (container != null && Optimizer.enabled(ConstraintLayout.this.mOptimizationLevel, 256) && child.getMeasuredWidth() == widget.getWidth() && child.getMeasuredWidth() < container.getWidth() && child.getMeasuredHeight() == widget.getHeight() && child.getMeasuredHeight() < container.getHeight() && child.getBaseline() == widget.getBaselineDistance() && !widget.isMeasureRequested()) {
                boolean similar = (isSimilarSpec(widget.getLastHorizontalMeasureSpec(), horizontalSpec4, widget.getWidth()) && isSimilarSpec(widget.getLastVerticalMeasureSpec(), verticalSpec, widget.getHeight())) ? ConstraintLayout.USE_CONSTRAINTS_HELPER : false;
                if (similar) {
            ...
            boolean horizontalUseRatio2 = (!horizontalMatchConstraints || widget.mDimensionRatio <= 0.0f) ? false : ConstraintLayout.USE_CONSTRAINTS_HELPER;
            if (verticalMatchConstraints) {
                horizontalUseRatio = horizontalUseRatio2;
                boolean verticalUseRatio = widget.mDimensionRatio > 0.0f ? ConstraintLayout.USE_CONSTRAINTS_HELPER : false;
                if (child != null) {
                    return;
                }
                boolean verticalUseRatio2 = verticalUseRatio;
                LayoutParams params = (LayoutParams) child.getLayoutParams();
                boolean horizontalDimensionKnown2 = horizontalDimensionKnown;
                if (measure.measureStrategy != BasicMeasure.Measure.TRY_GIVEN_DIMENSIONS && measure.measureStrategy != BasicMeasure.Measure.USE_GIVEN_DIMENSIONS && horizontalMatchConstraints && widget.mMatchConstraintDefaultWidth == 0 && verticalMatchConstraints && widget.mMatchConstraintDefaultHeight == 0) {
                    ...
                    if ((child instanceof VirtualLayout) && (widget instanceof androidx.constraintlayout.core.widgets.VirtualLayout)) {
                        ...
                        ((VirtualLayout) child).onMeasure(layout, horizontalSpec4, verticalSpec);
                    } else {
                        child.measure(horizontalSpec4, verticalSpec);
                    }
                    widget.setLastMeasureSpec(horizontalSpec4, verticalSpec);
                    int w2 = child.getMeasuredWidth();
                    int h = child.getMeasuredHeight();
                    int baseline = child.getBaseline();
                    int horizontalDimension2 = widget.mMatchConstraintMinWidth;
                    if (horizontalDimension2 > 0) {
                        int width = Math.max(widget.mMatchConstraintMinWidth, w2);
                        verticalDimension = width;
                    } else {
                        verticalDimension = w2;
                    }
                    if (widget.mMatchConstraintMaxWidth > 0) {
                        verticalDimension = Math.min(widget.mMatchConstraintMaxWidth, verticalDimension);
                    }
                    if (widget.mMatchConstraintMinHeight > 0) {
                        horizontalSpec = horizontalSpec4;
                    ...
                    if (!optimizeDirect) {
                        if (horizontalUseRatio && verticalDimensionKnown) {
                            ...
                            float ratio2 = height;
                            int width2 = (int) ((ratio2 * ratio) + 0.5f);
                            verticalDimension = width2;
                        } else if (verticalUseRatio2 && horizontalDimensionKnown2) {
                            float ratio3 = widget.mDimensionRatio;
                            float ratio4 = verticalDimension;
                            height = (int) ((ratio4 / ratio3) + 0.5f);
                        }
                    }
                    if (w2 == verticalDimension && h == height) {
                        horizontalSpec3 = height;
                        w = baseline;
                    } else {
                        if (w2 == verticalDimension) {
                            i = BasicMeasure.EXACTLY;
                            horizontalSpec2 = horizontalSpec;
                        } else {
                            i = BasicMeasure.EXACTLY;
                            horizontalSpec2 = View.MeasureSpec.makeMeasureSpec(verticalDimension, BasicMeasure.EXACTLY);
                        }
                        if (h != height) {
                            verticalSpec = View.MeasureSpec.makeMeasureSpec(height, i);
                        }
                        int horizontalSpec5 = horizontalSpec2;
                        child.measure(horizontalSpec5, verticalSpec);
                        widget.setLastMeasureSpec(horizontalSpec5, verticalSpec);
                        int width3 = child.getMeasuredWidth();
                        int height3 = child.getMeasuredHeight();
                        int baseline2 = child.getBaseline();
                        horizontalSpec3 = height3;
                        verticalDimension = width3;
                        w = baseline2;
                    }
                }
                boolean hasBaseline = w != -1 ? ConstraintLayout.USE_CONSTRAINTS_HELPER : false;
                measure.measuredNeedsSolverPass = (verticalDimension == measure.horizontalDimension && horizontalSpec3 == measure.verticalDimension) ? false : ConstraintLayout.USE_CONSTRAINTS_HELPER;
                ...
                measure.measuredWidth = verticalDimension;
                measure.measuredHeight = horizontalSpec3;
