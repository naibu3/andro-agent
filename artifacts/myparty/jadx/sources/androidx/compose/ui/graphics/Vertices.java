...
public final class Vertices {
    ...
    private final int[] encodeColorList(List<Color> colors) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m3711toArgb8_81llA(colors.get(i).m3667unboximpl());
        }
        return iArr;
    }
    ...
    private final float[] encodePointList(List<Offset> points) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i % 2 == 0) {
                fM3417getYimpl = Offset.m3416getXimpl(packedValue);
            } else {
                fM3417getYimpl = Offset.m3417getYimpl(packedValue);
            }
            fArr[i] = fM3417getYimpl;
        }
        return fArr;
    }
}
