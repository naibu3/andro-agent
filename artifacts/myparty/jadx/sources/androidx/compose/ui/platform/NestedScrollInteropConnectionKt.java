...
public final class NestedScrollInteropConnectionKt {
    ...
    public static final long m5380toOffsetUv8p0NA(int[] iArr, long j) {
        float fCoerceAtLeast;
        float fCoerceAtLeast2;
        if (Offset.m3416getXimpl(j) >= 0.0f) {
            fCoerceAtLeast = RangesKt.coerceAtMost(reverseAxis(iArr[0]), Offset.m3416getXimpl(j));
        } else {
            fCoerceAtLeast = RangesKt.coerceAtLeast(reverseAxis(iArr[0]), Offset.m3416getXimpl(j));
        }
        if (Offset.m3417getYimpl(j) >= 0.0f) {
            fCoerceAtLeast2 = RangesKt.coerceAtMost(reverseAxis(iArr[1]), Offset.m3417getYimpl(j));
        } else {
            fCoerceAtLeast2 = RangesKt.coerceAtLeast(reverseAxis(iArr[1]), Offset.m3417getYimpl(j));
        }
    ...
    public static final int m5381toViewTypeGyEprt8(int i) {
        return !NestedScrollSource.m4747equalsimpl0(i, NestedScrollSource.INSTANCE.m4759getUserInputWNlRxjI()) ? 1 : 0;
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* renamed from: getScrollAxes-k-4lQ0M, reason: not valid java name */
    public static final int m5379getScrollAxesk4lQ0M(long j) {
        int i = Math.abs(Offset.m3416getXimpl(j)) >= 0.5f ? 1 : 0;
        return Math.abs(Offset.m3417getYimpl(j)) >= 0.5f ? i | 2 : i;
    }
