...
public abstract class AbstractSendChannel<E> implements SendChannel<E> {
    ...
    public final Object mo1635trySendJP2dKIU(E element) {
        ...
        if (result == AbstractChannelKt.OFFER_FAILED) {
            ...
            return closedForSend == null ? ChannelResult.INSTANCE.m1654failurePtdJZtk() : ChannelResult.INSTANCE.m1653closedJP2dKIU(helpCloseAndGetSendException(closedForSend));
        }
        if (result instanceof Closed) {
            return ChannelResult.INSTANCE.m1653closedJP2dKIU(helpCloseAndGetSendException((Closed) result));
        }
    ...
    */
    ...
    public final void helpCloseAndResumeWithSendException(Continuation<?> continuation, E e, Closed<?> closed) {
        ...
        if (function1 != null && (it = OnUndeliveredElementKt.callUndeliveredElementCatchingException$default(function1, e, null, 2, null)) != null) {
            ...
            Result.Companion companion = Result.INSTANCE;
            continuation.resumeWith(Result.m130constructorimpl(ResultKt.createFailure(it)));
        } else {
            Result.Companion companion2 = Result.INSTANCE;
            continuation.resumeWith(Result.m130constructorimpl(ResultKt.createFailure(sendException)));
        }
    }
    ...
    */
    ...
    private final void helpClose(Closed<?> closed) {
        Object closedList = InlineList.m1672constructorimpl$default(null, 1, null);
        while (true) {
            LockFreeLinkedListNode prevNode = closed.getPrevNode();
            Receive previous = prevNode instanceof Receive ? (Receive) prevNode : null;
            if (previous == null) {
                break;
            } else if (!previous.mo1680remove()) {
                previous.helpRemove();
            } else {
                closedList = InlineList.m1677plusFjFbRPM(closedList, previous);
            }
        }
        if (closedList != null) {
