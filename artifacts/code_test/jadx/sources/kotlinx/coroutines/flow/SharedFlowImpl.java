...
public class SharedFlowImpl<T> extends AbstractSharedFlow<SharedFlowSlot> implements MutableSharedFlow<T>, CancellableFlow<T>, FusibleFlow<T> {
    ...
    /* JADX WARN: Removed duplicated region for block: B:32:0x00a9 A[Catch: all -> 0x00d7, TryCatch #0 {all -> 0x00d7, blocks: (B:13:0x003c, B:30:0x00a0, B:32:0x00a9, B:37:0x00be, B:38:0x00c1, B:16:0x0054, B:19:0x0066, B:28:0x008e, B:22:0x0076, B:24:0x007a), top: B:47:0x0021 }] */
            ...
            switch (anonymousClass1.label) {
                case 0:
                    ...
                    if (flowCollector2 instanceof SubscribedFlowCollector) {
                        anonymousClass1.L$0 = sharedFlowImpl3;
                        anonymousClass1.L$1 = flowCollector2;
                        anonymousClass1.L$2 = sharedFlowSlotAllocateSlot;
                        anonymousClass1.label = 1;
                        sharedFlowImpl2 = sharedFlowImpl3;
                        sharedFlowSlot = sharedFlowSlotAllocateSlot;
                        if (((SubscribedFlowCollector) flowCollector2).onSubscription(anonymousClass1) == coroutine_suspended) {
                    ...
                    do {
                        ...
                        if (objTryTakeValue == SharedFlowKt.NO_VALUE) {
                            ...
                            if (r2.emit(objTryTakeValue, anonymousClass1) == coroutine_suspended) {
                                return coroutine_suspended;
                            }
                    ...
                    } while (r3.awaitValue(flowCollector, anonymousClass1) != coroutine_suspended);
                    return coroutine_suspended;
                case 1:
                ...
                case 2:
                    ...
                    r2 = (FlowCollector) anonymousClass1.L$1;
                    r3 = (SharedFlowImpl) anonymousClass1.L$0;
                    ResultKt.throwOnFailure(obj);
                    do {
                        objTryTakeValue = r3.tryTakeValue(flowCollector);
                        if (objTryTakeValue == SharedFlowKt.NO_VALUE) {
                ...
                case 3:
                    ...
                    SharedFlowSlot sharedFlowSlot3 = (SharedFlowSlot) anonymousClass1.L$2;
    ...
    public boolean tryEmit(T value) {
        ...
        for (Continuation cont : continuationArrFindSlotsToResumeLocked) {
            if (cont != null) {
                ...
                cont.resumeWith(Result.m130constructorimpl(Unit.INSTANCE));
            }
        }
        return emitted;
    }
    ...
    public final Object emitSuspend(T t, Continuation<? super Unit> continuation) {
        ...
        for (Continuation r : continuationArrFindSlotsToResumeLocked) {
            if (r != null) {
                ...
                r.resumeWith(Result.m130constructorimpl(Unit.INSTANCE));
            }
        }
    ...
    */
    public final Continuation<Unit>[] updateCollectorIndexLocked$kotlinx_coroutines_core(long oldIndex) {
        ...
        long newQueueEndIndex2 = this.queueSize + newBufferEndIndex2;
        if (maxResumeCount <= 0) {
            ...
            while (true) {
                if (curEmitterIndex >= newQueueEndIndex2) {
                    newMinCollectorIndex = newMinCollectorIndex2;
                    resumes = resumes3;
                    break;
                }
        ...
        long newReplayIndex = Math.max(this.replayIndex, newBufferEndIndex3 - Math.min(this.replay, newBufferSize1));
        if (this.bufferCapacity == 0 && newReplayIndex < newQueueEndIndex2) {
            Object[] objArr = this.buffer;
            Intrinsics.checkNotNull(objArr);
            if (Intrinsics.areEqual(SharedFlowKt.getBufferAt(objArr, newReplayIndex), SharedFlowKt.NO_VALUE)) {
                newReplayIndex += j;
                newQueueEndIndex = newQueueEndIndex2;
                newBufferEndIndex = newBufferEndIndex3 + j;
            }
        } else {
            newQueueEndIndex = newQueueEndIndex2;
            newBufferEndIndex = newBufferEndIndex3;
        }
        updateBufferLocked(newReplayIndex, newMinCollectorIndex3, newBufferEndIndex, newQueueEndIndex);
        cleanupTailLocked();
        if (!(resumes.length == 0)) {
    ...
    private final Object tryTakeValue(SharedFlowSlot slot) {
        ...
        for (Continuation resume : continuationArrUpdateCollectorIndexLocked$kotlinx_coroutines_core) {
            if (resume != null) {
                Result.Companion companion = Result.INSTANCE;
                resume.resumeWith(Result.m130constructorimpl(Unit.INSTANCE));
            }
        }
        return value;
    }
