...
final class ConscryptEngine extends AbstractConscryptEngine implements NativeCrypto.SSLHandshakeCallbacks, SSLParametersImpl.AliasChooser, SSLParametersImpl.PSKCallbacks {
    ...
    private int readEncryptedData(ByteBuffer byteBuffer, int i2) throws SSLException {
        try {
            ...
            int iMin = Math.min(i2, byteBuffer.limit() - iPosition);
    ...
    private int readEncryptedDataHeap(ByteBuffer byteBuffer, int i2) {
        ...
        try {
            ...
            int encryptedDataDirect = readEncryptedDataDirect(orCreateLazyDirectBuffer, 0, Math.min(i2, orCreateLazyDirectBuffer.remaining()));
            if (encryptedDataDirect > 0) {
    ...
    private int readPlaintextDataHeap(ByteBuffer byteBuffer, int i2) {
        ...
        try {
            ...
            int plaintextDataDirect = readPlaintextDataDirect(orCreateLazyDirectBuffer, 0, Math.min(i2, orCreateLazyDirectBuffer.remaining()));
            if (plaintextDataDirect > 0) {
    ...
    private int writeEncryptedData(ByteBuffer byteBuffer, int i2) throws SSLException {
        try {
            int iPosition = byteBuffer.position();
            int iWriteEncryptedDataDirect = byteBuffer.isDirect() ? writeEncryptedDataDirect(byteBuffer, iPosition, i2) : writeEncryptedDataHeap(byteBuffer, iPosition, i2);
    ...
    private int writeEncryptedDataHeap(ByteBuffer byteBuffer, int i2, int i3) {
        ...
        try {
            BufferAllocator bufferAllocator = this.bufferAllocator;
            if (bufferAllocator != null) {
                allocatedBufferAllocateDirectBuffer = bufferAllocator.allocateDirectBuffer(i3);
                orCreateLazyDirectBuffer = allocatedBufferAllocateDirectBuffer.nioBuffer();
            } else {
                orCreateLazyDirectBuffer = getOrCreateLazyDirectBuffer();
            }
            int iLimit = byteBuffer.limit();
            int iMin = Math.min(Math.min(iLimit - i2, i3), orCreateLazyDirectBuffer.remaining());
            byteBuffer.limit(i2 + iMin);
    ...
    private int writePlaintextData(ByteBuffer byteBuffer, int i2) throws SSLException {
        try {
            int iPosition = byteBuffer.position();
            int iWritePlaintextDataDirect = byteBuffer.isDirect() ? writePlaintextDataDirect(byteBuffer, iPosition, i2) : writePlaintextDataHeap(byteBuffer, iPosition, i2);
    ...
    private int writePlaintextDataHeap(ByteBuffer byteBuffer, int i2, int i3) {
        ...
        try {
            ...
            int iLimit = byteBuffer.limit();
            int iMin = Math.min(i3, orCreateLazyDirectBuffer.remaining());
            byteBuffer.limit(i2 + iMin);
            orCreateLazyDirectBuffer.put(byteBuffer);
            orCreateLazyDirectBuffer.flip();
            byteBuffer.limit(iLimit);
    ...
    */
    public boolean isInboundDone() {
        ...
        synchronized (this.ssl) {
            try {
                int i2 = this.state;
                if (i2 == 8 || i2 == 6 || this.ssl.wasShutdownReceived()) {
                    z2 = pendingInboundCleartextBytes() == 0;
                }
            } finally {
            }
        }
    ...
    */
    public boolean isOutboundDone() {
        ...
        synchronized (this.ssl) {
            try {
                int i2 = this.state;
                if (i2 == 8 || i2 == 7 || this.ssl.wasShutdownSent()) {
                    z2 = pendingOutboundEncryptedBytes() == 0;
                }
            } finally {
            }
        }
    ...
    public SSLEngineResult wrap(ByteBuffer[] byteBufferArr, int i2, int i3, ByteBuffer byteBuffer) {
        ...
        if (!byteBuffer.isReadOnly()) {
            if (i2 != 0 || i3 != byteBufferArr.length) {
                byteBufferArr = (ByteBuffer[]) Arrays.copyOfRange(byteBufferArr, i2, i4);
            }
    ...
    public SSLEngineResult unwrap(ByteBuffer byteBuffer, ByteBuffer[] byteBufferArr, int i2, int i3) {
        ...
        synchronized (this.ssl) {
            try {
                try {
                    sSLEngineResultUnwrap = unwrap(singleSrcBuffer(byteBuffer), 0, 1, byteBufferArr, i2, i3);
                } finally {
                    resetSingleSrcBuffer();
                }
            } catch (Throwable th) {
    ...
    */
    public SSLEngineResult unwrap(ByteBuffer[] byteBufferArr, int i2, int i3, ByteBuffer[] byteBufferArr2, int i4, int i5) {
        ...
        Preconditions.checkArgument(byteBufferArr2 != null, "dsts is null");
        int i8 = i3 + i2;
        Preconditions.checkPositionIndexes(i2, i8, byteBufferArr.length);
        int i9 = i4 + i5;
        Preconditions.checkPositionIndexes(i4, i9, byteBufferArr2.length);
        int iCalcDstsLength = calcDstsLength(byteBufferArr2, i4, i5);
        long jCalcSrcsLength = calcSrcsLength(byteBufferArr, i2, i8);
        synchronized (this.ssl) {
            try {
                int i10 = this.state;
                if (i10 != 0) {
                    ...
                    if (jCalcSrcsLength <= 0 || !z2) {
                        ...
                        encryptedPacketLength = SSLUtils.getEncryptedPacketLength(byteBufferArr, i2);
                    ...
                    if (encryptedPacketLength <= 0 || i2 >= i8) {
                        i6 = 0;
                    } else {
                        i6 = 0;
                        while (true) {
                    ...
                    if (iCalcDstsLength <= 0) {
                        try {
                            ...
                            return newResult(i6, i, handshakeStatusInternal);
                        }
                    } else {
                        i7 = 0;
                        while (i4 < i9) {
                                ...
                                if (byteBuffer2.hasRemaining()) {
                                    ...
                                    if (plaintextData <= 0) {
                                        if (plaintextData != -6) {
                                            ...
                                            return newResult(i6, i7, handshakeStatusInternal);
                                        }
                                        closeAll();
                        ...
                        if ((this.handshakeFinished ? pendingInboundCleartextBytes() : 0) <= 0) {
                            ...
                            return new SSLEngineResult(status, mayFinishHandshake(handshakeStatusInternal), i6, i7);
                        }
                        return newResult(i6, i7, handshakeStatusInternal);
                    }
                } else {
