...
final class ParentSizeNode extends Modifier.Node implements LayoutModifierNode {
    ...
    public MeasureResult mo430measure3p2s80s(MeasureScope measureScope, Measurable measurable, long j) {
        ...
        int iM6072getMinWidthimpl = iRound != Integer.MAX_VALUE ? iRound : Constraints.m6072getMinWidthimpl(j);
        int iM6071getMinHeightimpl = iRound2 != Integer.MAX_VALUE ? iRound2 : Constraints.m6071getMinHeightimpl(j);
        if (iRound == Integer.MAX_VALUE) {
            iRound = Constraints.m6070getMaxWidthimpl(j);
        }
        if (iRound2 == Integer.MAX_VALUE) {
            iRound2 = Constraints.m6069getMaxHeightimpl(j);
        }
        final Placeable placeableMo4993measureBRTryo0 = measurable.mo4993measureBRTryo0(ConstraintsKt.Constraints(iM6072getMinWidthimpl, iRound, iM6071getMinHeightimpl, iRound2));
        return MeasureScope.layout$default(measureScope, placeableMo4993measureBRTryo0.getWidth(), placeableMo4993measureBRTryo0.getHeight(), null, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.lazy.ParentSizeNode$measure$1
            {
                super(1);
            }
