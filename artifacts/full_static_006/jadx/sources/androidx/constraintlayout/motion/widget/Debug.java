...
public class Debug {
    ...
    public static String getState(MotionLayout layout, int stateId, int len) throws Resources.NotFoundException {
        ...
        return (resourceEntryName.length() <= len || (length = resourceEntryName.replaceAll("[^_]", "").length()) <= 0) ? resourceEntryName : resourceEntryName.replaceAll(CharBuffer.allocate((resourceEntryName.length() - len) / length).toString().replace((char) 0, '.') + Constants.USER_ID_SEPARATOR, Constants.USER_ID_SEPARATOR);
    }
