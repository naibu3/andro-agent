...
public final class HitTestResult implements List<Modifier.Node>, KMappedMarker {
    ...
    public final boolean hasHit() {
        long jM5136findBestHitDistanceptXAw2c = m5136findBestHitDistanceptXAw2c();
        return DistanceAndInLayer.m5131getDistanceimpl(jM5136findBestHitDistanceptXAw2c) < 0.0f && DistanceAndInLayer.m5133isInLayerimpl(jM5136findBestHitDistanceptXAw2c);
    }
    ...
    public final boolean isHitInMinimumTouchTargetBetter(float distanceFromEdge, boolean isInLayer) {
        ...
        return DistanceAndInLayer.m5127compareToS_HNhKs(m5136findBestHitDistanceptXAw2c(), HitTestResultKt.DistanceAndInLayer(distanceFromEdge, isInLayer)) > 0;
    }

    /* renamed from: findBestHitDistance-ptXAw2c, reason: not valid java name */
    private final long m5136findBestHitDistanceptXAw2c() {
        long jDistanceAndInLayer = HitTestResultKt.DistanceAndInLayer(Float.POSITIVE_INFINITY, false);
        int i = this.hitDepth + 1;
        int lastIndex = CollectionsKt.getLastIndex(this);
        if (i <= lastIndex) {
            while (true) {
                long jM5128constructorimpl = DistanceAndInLayer.m5128constructorimpl(this.distanceFromEdgeAndInLayer[i]);
                if (DistanceAndInLayer.m5127compareToS_HNhKs(jM5128constructorimpl, jDistanceAndInLayer) < 0) {
                    jDistanceAndInLayer = jM5128constructorimpl;
                }
                if ((DistanceAndInLayer.m5131getDistanceimpl(jDistanceAndInLayer) < 0.0f && DistanceAndInLayer.m5133isInLayerimpl(jDistanceAndInLayer)) || i == lastIndex) {
                    break;
                }
                i++;
            }
    ...
    public final void speculativeHit(Modifier.Node node, float distanceFromEdge, boolean isInLayer, Function0<Unit> childHitTest) {
        ...
        hitInMinimumTouchTarget(node, distanceFromEdge, isInLayer, childHitTest);
        if (this.hitDepth + 1 < CollectionsKt.getLastIndex(hitTestResult) && DistanceAndInLayer.m5127compareToS_HNhKs(jM5136findBestHitDistanceptXAw2c, m5136findBestHitDistanceptXAw2c()) > 0) {
