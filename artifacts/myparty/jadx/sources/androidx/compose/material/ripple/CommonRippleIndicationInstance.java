...
final class CommonRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver {
    ...
    public void drawIndication(ContentDrawScope contentDrawScope) {
        float fM2100getRippleEndRadiuscSwnlzA;
        if (Float.isNaN(this.radius)) {
            fM2100getRippleEndRadiuscSwnlzA = RippleAnimationKt.m2100getRippleEndRadiuscSwnlzA(contentDrawScope, this.bounded, contentDrawScope.mo4213getSizeNHjbRc());
        } else {
            fM2100getRippleEndRadiuscSwnlzA = contentDrawScope.mo712toPx0680j_4(this.radius);
        }
        this.targetRadius = fM2100getRippleEndRadiuscSwnlzA;
        long jM3667unboximpl = this.color.getValue().m3667unboximpl();
        contentDrawScope.drawContent();
        ContentDrawScope contentDrawScope2 = contentDrawScope;
        m2105drawStateLayerH2RKhps(contentDrawScope2, this.radius, jM3667unboximpl);
        m2098drawRipples4WTKRHQ(contentDrawScope2, jM3667unboximpl);
    }
    ...
    public void addRipple(PressInteraction.Press interaction, CoroutineScope scope) {
        ...
        RippleAnimation rippleAnimation = new RippleAnimation(this.bounded ? Offset.m3405boximpl(interaction.getPressPosition()) : null, this.targetRadius, this.bounded, null);
    ...
    private final void m2098drawRipples4WTKRHQ(DrawScope drawScope, long j) {
        long j2;
        Iterator<Map.Entry<PressInteraction.Press, RippleAnimation>> it = this.ripples.entrySet().iterator();
        while (it.hasNext()) {
            ...
            if (pressedAlpha == 0.0f) {
                ...
                j2 = j;
                value.m2099draw4WTKRHQ(drawScope, Color.m3656copywmQWz5c$default(j2, pressedAlpha, 0.0f, 0.0f, 0.0f, 14, null));
            }
            j = j2;
        }
    }
