...
public final class EditProcessor {
    ...
    private TextFieldValue mBufferState = new TextFieldValue(AnnotatedStringKt.emptyAnnotatedString(), TextRange.INSTANCE.m5607getZerod9O1mEE(), (TextRange) null, (DefaultConstructorMarker) null);
    ...
    public final void reset(TextFieldValue value, TextInputSession textInputSession) {
        boolean zAreEqual = Intrinsics.areEqual(value.getComposition(), this.mBuffer.m5761getCompositionMzsxiRA$ui_text_release());
        boolean z = true;
        boolean z2 = false;
        ...
        } else if (TextRange.m5595equalsimpl0(this.mBufferState.getSelection(), value.getSelection())) {
            z = false;
        } else {
            this.mBuffer.setSelection$ui_text_release(TextRange.m5600getMinimpl(value.getSelection()), TextRange.m5599getMaximpl(value.getSelection()));
            z2 = true;
            z = false;
        }
        if (value.getComposition() == null) {
            this.mBuffer.commitComposition$ui_text_release();
        } else if (!TextRange.m5596getCollapsedimpl(value.getComposition().getPackedValue())) {
            this.mBuffer.setComposition$ui_text_release(TextRange.m5600getMinimpl(value.getComposition().getPackedValue()), TextRange.m5599getMaximpl(value.getComposition().getPackedValue()));
        }
        if (z || (!z2 && !zAreEqual)) {
            this.mBuffer.commitComposition$ui_text_release();
            value = TextFieldValue.m5844copy3r_uNRQ$default(value, (AnnotatedString) null, 0L, (TextRange) null, 3, (Object) null);
        }
    ...
    private final String generateBatchErrorMessage(List<? extends EditCommand> editCommands, final EditCommand failedCommand) {
        ...
        StringBuilder sbAppend = sb.append("Error while applying EditCommand batch to buffer (length=" + this.mBuffer.getLength$ui_text_release() + ", composition=" + this.mBuffer.m5761getCompositionMzsxiRA$ui_text_release() + ", selection=" + ((Object) TextRange.m5605toStringimpl(this.mBuffer.m5762getSelectiond9O1mEE$ui_text_release())) + "):");
    ...
    public final TextFieldValue apply(List<? extends EditCommand> editCommands) {
        ...
        try {
            ...
            long jM5762getSelectiond9O1mEE$ui_text_release = this.mBuffer.m5762getSelectiond9O1mEE$ui_text_release();
            TextRange textRangeM5590boximpl = TextRange.m5590boximpl(jM5762getSelectiond9O1mEE$ui_text_release);
            textRangeM5590boximpl.getPackedValue();
            TextRange textRange = TextRange.m5601getReversedimpl(this.mBufferState.getSelection()) ? null : textRangeM5590boximpl;
            TextFieldValue textFieldValue = new TextFieldValue(annotatedString$ui_text_release, textRange != null ? textRange.getPackedValue() : TextRangeKt.TextRange(TextRange.m5599getMaximpl(jM5762getSelectiond9O1mEE$ui_text_release), TextRange.m5600getMinimpl(jM5762getSelectiond9O1mEE$ui_text_release)), this.mBuffer.m5761getCompositionMzsxiRA$ui_text_release(), (DefaultConstructorMarker) null);
