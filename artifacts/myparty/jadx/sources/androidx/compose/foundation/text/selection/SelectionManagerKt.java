...
public final class SelectionManagerKt {
    ...
    public static final Rect getSelectedRegionRect(List<? extends Pair<? extends Selectable, Selection>> list, LayoutCoordinates layoutCoordinates) {
        ...
        while (i2 < size) {
            ...
            if (offset == offset2 || (layoutCoordinates2 = selectableComponent1.getLayoutCoordinates()) == null) {
                ...
                long jMo5002localPositionOfR5De75A = layoutCoordinates.mo5002localPositionOfR5De75A(layoutCoordinates2, jOffset);
                long jMo5002localPositionOfR5De75A2 = layoutCoordinates.mo5002localPositionOfR5De75A(layoutCoordinates2, jOffset2);
                left = Math.min(left, Offset.m3416getXimpl(jMo5002localPositionOfR5De75A));
                top = Math.min(top, Offset.m3417getYimpl(jMo5002localPositionOfR5De75A));
                right = Math.max(right, Offset.m3416getXimpl(jMo5002localPositionOfR5De75A2));
                bottom = Math.max(bottom, Offset.m3417getYimpl(jMo5002localPositionOfR5De75A2));
            }
            i2++;
            size = i;
            c = 0;
    ...
    public static final long m1719calculateSelectionMagnifierCenterAndroidO0kMr_c(SelectionManager selectionManager, long j) {
        ...
        if (i == 1) {
            return m1721getMagnifierCenterJVtK1S4(selectionManager, j, selection.getStart());
        }
        if (i == 2) {
            return m1721getMagnifierCenterJVtK1S4(selectionManager, j, selection.getEnd());
        }
        if (i == 3) {
    ...
    private static final long m1721getMagnifierCenterJVtK1S4(SelectionManager selectionManager, long j, Selection.AnchorInfo anchorInfo) {
        ...
        if (anchorSelectable$foundation_release != null && (containerLayoutCoordinates = selectionManager.getContainerLayoutCoordinates()) != null && (layoutCoordinates = anchorSelectable$foundation_release.getLayoutCoordinates()) != null && (offset = anchorInfo.getOffset()) <= anchorSelectable$foundation_release.getLastVisibleOffset()) {
            Offset offsetM1708getCurrentDragPosition_m7T9E = selectionManager.m1708getCurrentDragPosition_m7T9E();
            Intrinsics.checkNotNull(offsetM1708getCurrentDragPosition_m7T9E);
            float fM3416getXimpl = Offset.m3416getXimpl(layoutCoordinates.mo5002localPositionOfR5De75A(containerLayoutCoordinates, offsetM1708getCurrentDragPosition_m7T9E.getPackedValue()));
            long jMo1669getRangeOfLineContainingjx7JFs = anchorSelectable$foundation_release.mo1669getRangeOfLineContainingjx7JFs(offset);
            if (TextRange.m5596getCollapsedimpl(jMo1669getRangeOfLineContainingjx7JFs)) {
                fCoerceIn = anchorSelectable$foundation_release.getLineLeft(offset);
            } else {
                float lineLeft = anchorSelectable$foundation_release.getLineLeft(TextRange.m5602getStartimpl(jMo1669getRangeOfLineContainingjx7JFs));
                float lineRight = anchorSelectable$foundation_release.getLineRight(TextRange.m5597getEndimpl(jMo1669getRangeOfLineContainingjx7JFs) - 1);
                fCoerceIn = RangesKt.coerceIn(fM3416getXimpl, Math.min(lineLeft, lineRight), Math.max(lineLeft, lineRight));
            }
            if (fCoerceIn == -1.0f) {
                return Offset.INSTANCE.m3431getUnspecifiedF1C5BW0();
            }
            if (!IntSize.m6285equalsimpl0(j, IntSize.INSTANCE.m6292getZeroYbymL2g()) && Math.abs(fM3416getXimpl - fCoerceIn) > IntSize.m6287getWidthimpl(j) / 2) {
                return Offset.INSTANCE.m3431getUnspecifiedF1C5BW0();
            }
            ...
            return containerLayoutCoordinates.mo5002localPositionOfR5De75A(layoutCoordinates, OffsetKt.Offset(fCoerceIn, centerYForOffset));
        }
        return Offset.INSTANCE.m3431getUnspecifiedF1C5BW0();
    }
    ...
    public static final Rect visibleBounds(LayoutCoordinates layoutCoordinates) {
        ...
        return RectKt.m3454Rect0a9Yr6o(layoutCoordinates.mo5010windowToLocalMKHz9U(rectBoundsInWindow.m3451getTopLeftF1C5BW0()), layoutCoordinates.mo5010windowToLocalMKHz9U(rectBoundsInWindow.m3445getBottomRightF1C5BW0()));
    }

    /* renamed from: containsInclusive-Uv8p0NA, reason: not valid java name */
    public static final boolean m1720containsInclusiveUv8p0NA(Rect rect, long j) {
        float left = rect.getLeft();
        float right = rect.getRight();
        float fM3416getXimpl = Offset.m3416getXimpl(j);
        if (left > fM3416getXimpl || fM3416getXimpl > right) {
            return false;
        }
        float top = rect.getTop();
        float bottom = rect.getBottom();
        float fM3417getYimpl = Offset.m3417getYimpl(j);
        return top <= fM3417getYimpl && fM3417getYimpl <= bottom;
    }
}
