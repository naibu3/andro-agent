...
public final class AztecWriter implements Writer {
    ...
    @Override // com.google.zxing.Writer
    public BitMatrix encode(String contents, BarcodeFormat format, int width, int height, Map<EncodeHintType, ?> hints) throws NumberFormatException {
        Charset charset;
        int eccPercent;
        int layers;
        ...
        if (hints == null) {
            ...
            if (hints.containsKey(EncodeHintType.CHARACTER_SET)) {
                charset2 = Charset.forName(hints.get(EncodeHintType.CHARACTER_SET).toString());
            }
            if (hints.containsKey(EncodeHintType.ERROR_CORRECTION)) {
                eccPercent2 = Integer.parseInt(hints.get(EncodeHintType.ERROR_CORRECTION).toString());
            }
            if (!hints.containsKey(EncodeHintType.AZTEC_LAYERS)) {
                charset = charset2;
                eccPercent = eccPercent2;
                layers = 0;
            } else {
                int layers2 = Integer.parseInt(hints.get(EncodeHintType.AZTEC_LAYERS).toString());
                charset = charset2;
                eccPercent = eccPercent2;
                layers = layers2;
            }
