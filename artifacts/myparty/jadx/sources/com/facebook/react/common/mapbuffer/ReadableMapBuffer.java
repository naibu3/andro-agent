...
public final class ReadableMapBuffer extends HybridClassBase implements MapBuffer {
    ...
    private final void readHeader() {
        ...
        this.count = m6844readUnsignedShortBwKQO78(this.buffer.position()) & UShort.MAX_VALUE;
    }
    ...
    private final int getBucketIndexForKey(int intKey) {
        ...
        if (intKey <= kEY_RANGE$ReactAndroid_release.getLast() && first <= intKey) {
            short sM9403constructorimpl = UShort.m9403constructorimpl((short) intKey);
            int count = getCount() - 1;
            int i = 0;
            while (i <= count) {
                int i2 = (i + count) >>> 1;
                int iM6844readUnsignedShortBwKQO78 = m6844readUnsignedShortBwKQO78(getKeyOffsetForBucketIndex(i2)) & UShort.MAX_VALUE;
                int i3 = 65535 & sM9403constructorimpl;
                if (Intrinsics.compare(iM6844readUnsignedShortBwKQO78, i3) < 0) {
                    i = i2 + 1;
                } else {
                    if (Intrinsics.compare(iM6844readUnsignedShortBwKQO78, i3) <= 0) {
                        return i2;
    ...
    private final MapBuffer.DataType readDataType(int bucketIndex) {
        int iM6844readUnsignedShortBwKQO78 = m6844readUnsignedShortBwKQO78(getKeyOffsetForBucketIndex(bucketIndex) + 2) & UShort.MAX_VALUE;
        if (ReactNativeFeatureFlags.enableAndroidTextMeasurementOptimizations()) {
            return DATA_TYPES[iM6844readUnsignedShortBwKQO78];
        }
        return MapBuffer.DataType.values()[iM6844readUnsignedShortBwKQO78];
    }
    ...
    public final short m6844readUnsignedShortBwKQO78(int bufferPosition) {
        return UShort.m9403constructorimpl(this.buffer.getShort(bufferPosition));
    }
    ...
    private final class MapBufferEntry implements MapBuffer.Entry {
        ...
        public int getKey() {
            return ReadableMapBuffer.this.m6844readUnsignedShortBwKQO78(this.bucketOffset) & UShort.MAX_VALUE;
        }

        @Override // com.facebook.react.common.mapbuffer.MapBuffer.Entry
        public MapBuffer.DataType getType() {
            return ReactNativeFeatureFlags.enableAndroidTextMeasurementOptimizations() ? ReadableMapBuffer.DATA_TYPES[65535 & ReadableMapBuffer.this.m6844readUnsignedShortBwKQO78(this.bucketOffset + 2)] : MapBuffer.DataType.values()[65535 & ReadableMapBuffer.this.m6844readUnsignedShortBwKQO78(this.bucketOffset + 2)];
        }
