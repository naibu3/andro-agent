...
public final class LazyStaggeredGridMeasuredItem implements LazyStaggeredGridItemInfo, LazyLayoutMeasuredItem {
    ...
    private LazyStaggeredGridMeasuredItem(int i, Object obj, List<? extends Placeable> list, boolean z, int i2, int i3, int i4, int i5, int i6, Object obj2, LazyLayoutItemAnimator<LazyStaggeredGridMeasuredItem> lazyLayoutItemAnimator, long j) {
        ...
        this.size = jIntSize;
        this.offset = IntOffset.INSTANCE.m6255getZeronOccac();
    }
    ...
    public final int getMainAxisOffset() {
        return !getIsVertical() ? IntOffset.m6245getXimpl(getOffset()) : IntOffset.m6246getYimpl(getOffset());
    }
    ...
    public final void place(Placeable.PlacementScope scope, LazyStaggeredGridMeasureContext context) {
        ...
        while (i < size) {
            ...
            if (animation != null) {
                long jM6249plusqkQi6aY = IntOffset.m6249plusqkQi6aY(offset, animation.m1179getPlacementDeltanOccac());
                if ((m1237getMainAxisgyyYBs(offset) <= height && m1237getMainAxisgyyYBs(jM6249plusqkQi6aY) <= height) || (m1237getMainAxisgyyYBs(offset) >= i2 && m1237getMainAxisgyyYBs(jM6249plusqkQi6aY) >= i2)) {
                    animation.cancelPlacementAnimation();
                }
                layer = animation.getLayer();
                offset = jM6249plusqkQi6aY;
            } else {
                layer = null;
            }
            if (context.getReverseLayout()) {
                if (getIsVertical()) {
                    iM6245getXimpl = IntOffset.m6245getXimpl(offset);
                } else {
                    iM6245getXimpl = (this.mainAxisLayoutSize - IntOffset.m6245getXimpl(offset)) - (getIsVertical() ? placeable.getHeight() : placeable.getWidth());
                }
                if (!getIsVertical()) {
                    iM6246getYimpl = IntOffset.m6246getYimpl(offset);
                } else {
                    iM6246getYimpl = (this.mainAxisLayoutSize - IntOffset.m6246getYimpl(offset)) - (getIsVertical() ? placeable.getHeight() : placeable.getWidth());
                }
                offset = IntOffsetKt.IntOffset(iM6245getXimpl, iM6246getYimpl);
            }
            long jM6249plusqkQi6aY2 = IntOffset.m6249plusqkQi6aY(offset, context.getContentOffset());
            if (animation != null) {
                animation.m1181setFinalOffsetgyyYBs(jM6249plusqkQi6aY2);
            }
            if (layer != null) {
                placementScope = scope;
                Placeable.PlacementScope.m5060placeRelativeWithLayeraW9wM$default(placementScope, placeable, jM6249plusqkQi6aY2, layer, 0.0f, 4, (Object) null);
            } else {
                placementScope = scope;
    ...
    public final void applyScrollDelta(int delta) {
        ...
        long offset = getOffset();
        int iM6245getXimpl = getIsVertical() ? IntOffset.m6245getXimpl(offset) : IntOffset.m6245getXimpl(offset) + delta;
        boolean isVertical = getIsVertical();
        int iM6246getYimpl = IntOffset.m6246getYimpl(offset);
        if (isVertical) {
            iM6246getYimpl += delta;
        }
        this.offset = IntOffsetKt.IntOffset(iM6245getXimpl, iM6246getYimpl);
        int placeablesCount = getPlaceablesCount();
        for (int i = 0; i < placeablesCount; i++) {
            LazyLayoutItemAnimation animation = this.animator.getAnimation(getKey(), i);
            if (animation != null) {
                long jM1180getRawOffsetnOccac = animation.getRawOffset();
                int iM6245getXimpl2 = getIsVertical() ? IntOffset.m6245getXimpl(jM1180getRawOffsetnOccac) : Integer.valueOf(IntOffset.m6245getXimpl(jM1180getRawOffsetnOccac) + delta).intValue();
                boolean isVertical2 = getIsVertical();
                int iM6246getYimpl2 = IntOffset.m6246getYimpl(jM1180getRawOffsetnOccac);
                if (isVertical2) {
                    iM6246getYimpl2 += delta;
                }
                animation.m1183setRawOffsetgyyYBs(IntOffsetKt.IntOffset(iM6245getXimpl2, iM6246getYimpl2));
            }
        }
    }

    /* renamed from: getMainAxis--gyyYBs, reason: not valid java name */
    private final int m1237getMainAxisgyyYBs(long j) {
