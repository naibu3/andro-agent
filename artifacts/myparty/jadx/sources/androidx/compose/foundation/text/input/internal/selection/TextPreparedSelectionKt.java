...
public final class TextPreparedSelectionKt {
    ...
    public static final int calculateAdjacentCursorPosition(String str, int i, boolean z, TransformedTextFieldState transformedTextFieldState) {
        ...
        long jM1568mapFromTransformedjx7JFs = transformedTextFieldState.m1568mapFromTransformedjx7JFs(iFindPrecedingBreak);
        ...
        if (i2 == 3) {
            return z ? TextRange.m5597getEndimpl(jM1571mapToTransformedGEjPoXI) : TextRange.m5602getStartimpl(jM1571mapToTransformedGEjPoXI);
        }
        if (i2 != 4) {
            throw new NoWhenBranchMatchedException();
        }
        if (z) {
            if (iFindPrecedingBreak == TextRange.m5602getStartimpl(jM1571mapToTransformedGEjPoXI)) {
        ...
        if (iFindPrecedingBreak == TextRange.m5597getEndimpl(jM1571mapToTransformedGEjPoXI)) {
            transformedTextFieldState.setSelectionWedgeAffinity(new SelectionWedgeAffinity(WedgeAffinity.End));
            return iFindPrecedingBreak;
        }
        transformedTextFieldState.setSelectionWedgeAffinity(new SelectionWedgeAffinity(WedgeAffinity.Start));
