...
public final class TextFieldTextLayoutModifierNode extends Modifier.Node implements LayoutModifierNode, GlobalPositionAwareModifierNode, CompositionLocalConsumerModifierNode {
    ...
    public MeasureResult mo430measure3p2s80s(MeasureScope measureScope, Measurable measurable, long j) {
        ...
        TextLayoutResult textLayoutResultM1558layoutWithNewMeasureInputshBUhpc = this.textLayoutState.m1558layoutWithNewMeasureInputshBUhpc(measureScope, measureScope.getLayoutDirection(), (FontFamily.Resolver) CompositionLocalConsumerModifierNodeKt.currentValueOf(this, CompositionLocalsKt.getLocalFontFamilyResolver()), j);
        final Placeable placeableMo4993measureBRTryo0 = measurable.mo4993measureBRTryo0(Constraints.INSTANCE.m6079fitPrioritizingWidthZbe2FdA(IntSize.m6287getWidthimpl(textLayoutResultM1558layoutWithNewMeasureInputshBUhpc.getSize()), IntSize.m6287getWidthimpl(textLayoutResultM1558layoutWithNewMeasureInputshBUhpc.getSize()), IntSize.m6286getHeightimpl(textLayoutResultM1558layoutWithNewMeasureInputshBUhpc.getSize()), IntSize.m6286getHeightimpl(textLayoutResultM1558layoutWithNewMeasureInputshBUhpc.getSize())));
        TextLayoutState textLayoutState = this.textLayoutState;
        if (this.singleLine) {
            fM6117constructorimpl = measureScope.mo709toDpu2uoSUM(TextDelegateKt.ceilToIntPx(textLayoutResultM1558layoutWithNewMeasureInputshBUhpc.getLineBottom(0)));
        } else {
            fM6117constructorimpl = Dp.m6117constructorimpl(0);
        }
        textLayoutState.m1559setMinHeightForSingleLineField0680j_4(fM6117constructorimpl);
        LinkedHashMap linkedHashMap = this.baselineCache;
        if (linkedHashMap == null) {
            linkedHashMap = new LinkedHashMap(2);
        }
        linkedHashMap.put(AlignmentLineKt.getFirstBaseline(), Integer.valueOf(Math.round(textLayoutResultM1558layoutWithNewMeasureInputshBUhpc.getFirstBaseline())));
        linkedHashMap.put(AlignmentLineKt.getLastBaseline(), Integer.valueOf(Math.round(textLayoutResultM1558layoutWithNewMeasureInputshBUhpc.getLastBaseline())));
        this.baselineCache = linkedHashMap;
        int iM6287getWidthimpl = IntSize.m6287getWidthimpl(textLayoutResultM1558layoutWithNewMeasureInputshBUhpc.getSize());
        int iM6286getHeightimpl = IntSize.m6286getHeightimpl(textLayoutResultM1558layoutWithNewMeasureInputshBUhpc.getSize());
        Map<AlignmentLine, Integer> map = this.baselineCache;
        Intrinsics.checkNotNull(map);
        return measureScope.layout(iM6287getWidthimpl, iM6286getHeightimpl, map, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.text.input.internal.TextFieldTextLayoutModifierNode$measure$1
            {
                super(1);
            }
