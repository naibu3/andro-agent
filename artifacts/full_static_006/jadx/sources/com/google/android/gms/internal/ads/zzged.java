package com.google.android.gms.internal.ads;

import com.google.common.util.concurrent.ListenableFuture;

/* compiled from: com.google.android.gms:play-services-ads@@23.2.0 */
/* loaded from: classes2.dex */
final class zzged extends zzgef {
    zzged(ListenableFuture listenableFuture, Class cls, zzgfa zzgfaVar) {
        super(listenableFuture, cls, zzgfaVar);
    }

    @Override // com.google.android.gms.internal.ads.zzgef
    final /* bridge */ /* synthetic */ Object zze(Object obj, Throwable th) throws Exception {
        zzgfa zzgfaVar = (zzgfa) obj;
        ListenableFuture listenableFutureZza = zzgfaVar.zza(th);
        zzfyg.zzd(listenableFutureZza, "AsyncFunction.apply returned null instead of a Future. Did you mean to return immediateFuture(null)? %s", zzgfaVar);
        return listenableFutureZza;
    }

    @Override // com.google.android.gms.internal.ads.zzgef
    final /* synthetic */ void zzf(Object obj) {
        zzs((ListenableFuture) obj);
    }
}
