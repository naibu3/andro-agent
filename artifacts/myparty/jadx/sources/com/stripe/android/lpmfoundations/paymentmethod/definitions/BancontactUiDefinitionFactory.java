...
final class BancontactUiDefinitionFactory implements UiDefinitionFactory.RequiresSharedDataSpec {
    ...
    public SupportedPaymentMethod createSupportedPaymentMethod(PaymentMethodMetadata metadata, SharedDataSpec sharedDataSpec) {
        ...
        return new SupportedPaymentMethod((PaymentMethodDefinition) BancontactDefinition.INSTANCE, sharedDataSpec, R.string.stripe_paymentsheet_payment_method_bancontact, R.drawable.stripe_ic_paymentsheet_pm_bancontact, (Integer) null, false, (ResolvableString) null, (Integer) null, 224, (DefaultConstructorMarker) null);
    }
    ...
    public List<FormElement> createFormElements(PaymentMethodMetadata metadata, SharedDataSpec sharedDataSpec, TransformSpecToElements transformSpecToElements) {
        List<IdentifierSpec> listEmptyList;
        Intrinsics.checkNotNullParameter(metadata, "metadata");
        Intrinsics.checkNotNullParameter(sharedDataSpec, "sharedDataSpec");
        ...
        if (metadata.hasIntentToSetup(BancontactDefinition.INSTANCE.getType().code)) {
            listEmptyList = CollectionsKt.listOf((Object[]) new IdentifierSpec[]{IdentifierSpec.INSTANCE.getName(), IdentifierSpec.INSTANCE.getEmail()});
        } else {
            listEmptyList = CollectionsKt.emptyList();
        }
        return transformSpecToElements.transform(metadata, fields, listEmptyList);
    }
}
