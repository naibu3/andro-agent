...
public final class LazyGridAnimateScrollScope implements LazyLayoutAnimateScrollScope {
    ...
    public float calculateDistanceTo(int targetIndex) {
        ...
        if (layoutInfo.getOrientation() == Orientation.Vertical) {
            iM6245getXimpl = IntOffset.m6246getYimpl(lazyGridItemInfo2.getOffset());
        } else {
            iM6245getXimpl = IntOffset.m6245getXimpl(lazyGridItemInfo2.getOffset());
        }
        ...
        while (i < visibleItemsInfo.size()) {
            ...
            if (iIntValue == -1) {
                ...
                while (i < visibleItemsInfo.size() && function1.invoke(Integer.valueOf(i)).intValue() == iIntValue) {
                    if (z) {
                        iM6287getWidthimpl = IntSize.m6286getHeightimpl(visibleItemsInfo.get(i).getSize());
                    } else {
                        iM6287getWidthimpl = IntSize.m6287getWidthimpl(visibleItemsInfo.get(i).getSize());
                    }
                    iMax = Math.max(iMax, iM6287getWidthimpl);
                    i++;
                }
                i2 += iMax;
                i3++;
