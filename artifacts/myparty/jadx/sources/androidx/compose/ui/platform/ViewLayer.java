...
public final class ViewLayer extends View implements OwnedLayer, GraphicLayerInfo {
    ...
    public ViewLayer(AndroidComposeView androidComposeView, DrawChildContainer drawChildContainer, Function2<? super Canvas, ? super GraphicsLayer, Unit> function2, Function0<Unit> function0) {
        ...
        this.mTransformOrigin = TransformOrigin.INSTANCE.m4048getCenterSzJe1aQ();
    ...
    public void updateLayerProperties(ReusableGraphicsLayerScope scope) {
        ...
        if ((mutatedFields$ui_release & 4096) != 0) {
            long jMo3846getTransformOriginSzJe1aQ = scope.getTransformOrigin();
            this.mTransformOrigin = jMo3846getTransformOriginSzJe1aQ;
            setPivotX(TransformOrigin.m4043getPivotFractionXimpl(jMo3846getTransformOriginSzJe1aQ) * getWidth());
            setPivotY(TransformOrigin.m4044getPivotFractionYimpl(this.mTransformOrigin) * getHeight());
        }
        ...
        boolean zM5384updateS_szKao = this.outlineResolver.m5384updateS_szKao(scope.getOutline(), scope.getAlpha(), z3, scope.getShadowElevation(), scope.getSize());
        if (this.outlineResolver.getCacheIsDirty()) {
            updateOutlineResolver();
        }
        boolean z4 = getManualClipPath() != null;
        if (z2 != z4 || (z4 && zM5384updateS_szKao)) {
            invalidate();
        }
        ...
        if (Build.VERSION.SDK_INT >= 28) {
            if ((mutatedFields$ui_release & 64) != 0) {
                ViewLayerVerificationHelper28.INSTANCE.setOutlineAmbientShadowColor(this, ColorKt.m3711toArgb8_81llA(scope.getAmbientShadowColor()));
            }
            if ((mutatedFields$ui_release & 128) != 0) {
                ViewLayerVerificationHelper28.INSTANCE.setOutlineSpotShadowColor(this, ColorKt.m3711toArgb8_81llA(scope.getSpotShadowColor()));
            }
        }
        ...
        if ((mutatedFields$ui_release & 32768) != 0) {
            int iMo3843getCompositingStrategyNrFUSI = scope.getCompositingStrategy();
            if (CompositingStrategy.m3742equalsimpl0(iMo3843getCompositingStrategyNrFUSI, CompositingStrategy.INSTANCE.m3748getOffscreenNrFUSI())) {
                setLayerType(2, null);
            } else if (CompositingStrategy.m3742equalsimpl0(iMo3843getCompositingStrategyNrFUSI, CompositingStrategy.INSTANCE.m3747getModulateAlphaNrFUSI())) {
                setLayerType(0, null);
    ...
    public boolean mo5276isInLayerk4lQ0M(long position) {
        ...
        if (this.clipToBounds) {
            return 0.0f <= fM3416getXimpl && fM3416getXimpl < ((float) getWidth()) && 0.0f <= fM3417getYimpl && fM3417getYimpl < ((float) getHeight());
        }
        if (getClipToOutline()) {
            return this.outlineResolver.m5383isInOutlinek4lQ0M(position);
        }
        return true;
    }
    ...
    public void mo5279resizeozmzZPI(long size) {
        int iM6287getWidthimpl = IntSize.m6287getWidthimpl(size);
        int iM6286getHeightimpl = IntSize.m6286getHeightimpl(size);
        if (iM6287getWidthimpl == getWidth() && iM6286getHeightimpl == getHeight()) {
            return;
        }
        setPivotX(TransformOrigin.m4043getPivotFractionXimpl(this.mTransformOrigin) * iM6287getWidthimpl);
        setPivotY(TransformOrigin.m4044getPivotFractionYimpl(this.mTransformOrigin) * iM6286getHeightimpl);
        updateOutlineResolver();
        layout(getLeft(), getTop(), getLeft() + iM6287getWidthimpl, getTop() + iM6286getHeightimpl);
        resetClipBounds();
        this.matrixCache.invalidate();
    }
    ...
    public void mo5278movegyyYBs(long position) {
        int iM6245getXimpl = IntOffset.m6245getXimpl(position);
        if (iM6245getXimpl != getLeft()) {
            offsetLeftAndRight(iM6245getXimpl - getLeft());
            this.matrixCache.invalidate();
        }
        int iM6246getYimpl = IntOffset.m6246getYimpl(position);
        if (iM6246getYimpl != getTop()) {
            offsetTopAndBottom(iM6246getYimpl - getTop());
            this.matrixCache.invalidate();
        }
    }
    ...
    public long mo5277mapOffset8S9VItk(long point, boolean inverse) {
        if (inverse) {
            float[] fArrM5360calculateInverseMatrixbWbORWo = this.matrixCache.m5360calculateInverseMatrixbWbORWo(this);
            return fArrM5360calculateInverseMatrixbWbORWo != null ? androidx.compose.ui.graphics.Matrix.m3894mapMKHz9U(fArrM5360calculateInverseMatrixbWbORWo, point) : Offset.INSTANCE.m3430getInfiniteF1C5BW0();
        }
        return androidx.compose.ui.graphics.Matrix.m3894mapMKHz9U(this.matrixCache.m5361calculateMatrixGrdbGEg(this), point);
    }
    ...
    public void mapBounds(MutableRect rect, boolean inverse) {
        if (inverse) {
            float[] fArrM5360calculateInverseMatrixbWbORWo = this.matrixCache.m5360calculateInverseMatrixbWbORWo(this);
            if (fArrM5360calculateInverseMatrixbWbORWo != null) {
                androidx.compose.ui.graphics.Matrix.m3896mapimpl(fArrM5360calculateInverseMatrixbWbORWo, rect);
                return;
            } else {
        ...
        androidx.compose.ui.graphics.Matrix.m3896mapimpl(this.matrixCache.m5361calculateMatrixGrdbGEg(this), rect);
    }
    ...
    public void mo5280transform58bKbWc(float[] matrix) {
        androidx.compose.ui.graphics.Matrix.m3905timesAssign58bKbWc(matrix, this.matrixCache.m5361calculateMatrixGrdbGEg(this));
    }
    ...
    public void mo5275inverseTransform58bKbWc(float[] matrix) {
        float[] fArrM5360calculateInverseMatrixbWbORWo = this.matrixCache.m5360calculateInverseMatrixbWbORWo(this);
        if (fArrM5360calculateInverseMatrixbWbORWo != null) {
            androidx.compose.ui.graphics.Matrix.m3905timesAssign58bKbWc(matrix, fArrM5360calculateInverseMatrixbWbORWo);
        }
    }
