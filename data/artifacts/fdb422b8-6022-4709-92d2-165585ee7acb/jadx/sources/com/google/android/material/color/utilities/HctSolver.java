...
public class HctSolver {
    ...
    static double chromaticAdaptation(double component) {
        ...
        double dSignum = MathUtils.signum(component);
        Double.isNaN(dSignum);
        return ((dSignum * 400.0d) * af) / (27.13d + af);
    }
        ...
        for (int axis = 0; axis < 3; axis++) {
            if (left[axis] != right[axis]) {
                ...
                for (int i = 0; i < 8 && Math.abs(rPlane2 - lPlane2) > 1; i++) {
                    double d2 = lPlane2 + rPlane2;
                    Double.isNaN(d2);
                    int mPlane = (int) Math.floor(d2 / 2.0d);
    ...
    static double inverseChromaticAdaptation(double adapted) {
        ...
        double dSignum = MathUtils.signum(adapted);
        double dPow = Math.pow(base, 2.380952380952381d);
        Double.isNaN(dSignum);
        return dSignum * dPow;
    }
