...
public final class SpringAnimation extends AnimationDriver {
    ...
    private final boolean isAtRest() {
        if (Math.abs(this.currentState.getVelocity()) <= this.restSpeedThreshold) {
            return getDisplacementDistanceForState(this.currentState) <= this.displacementFromRestThreshold || this.springStiffness == AudioStats.AUDIO_AMPLITUDE_NONE;
        }
        return false;
    }
