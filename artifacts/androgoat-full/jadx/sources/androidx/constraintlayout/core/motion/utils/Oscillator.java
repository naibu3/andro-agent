...
public class Oscillator {
    ...
    public void normalize() {
        ...
        for (int i3 = 0; i3 < this.mPeriod.length; i3++) {
            ...
            double d3 = fArr[i3];
            Double.isNaN(d3);
            fArr[i3] = (float) (d3 * (totalCount / totalArea));
        }
        this.mArea[0] = 0.0d;
        for (int i4 = 1; i4 < this.mPeriod.length; i4++) {
            ...
            double[] dArr = this.mArea;
            double d4 = this.mArea[i4 - 1];
            double d5 = h2;
            Double.isNaN(d5);
            dArr[i4] = d4 + (d5 * w2);
        }
    ...
    double getP(double time) {
        ...
        int index2 = (-index) - 1;
        double t = time2;
        double d = this.mPeriod[index2] - this.mPeriod[index2 - 1];
        double d2 = this.mPosition[index2] - this.mPosition[index2 - 1];
        Double.isNaN(d);
        double m = d / d2;
        double d3 = this.mArea[index2 - 1];
        double d4 = this.mPeriod[index2 - 1];
        double d5 = this.mPosition[index2 - 1] * m;
        Double.isNaN(d4);
        double p = d3 + ((d4 - d5) * (t - this.mPosition[index2 - 1])) + ((((t * t) - (this.mPosition[index2 - 1] * this.mPosition[index2 - 1])) * m) / 2.0d);
        return p;
    }
    ...
    double getDP(double time) {
        ...
        Double.isNaN(d);
        double m = d / d2;
        double d3 = this.mPeriod[index2 - 1];
        double d4 = this.mPosition[index2 - 1] * m;
        Double.isNaN(d3);
        double p = (m * t) + (d3 - d4);
        return p;
    }
