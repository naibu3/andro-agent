...
public final class MultiWidgetSelectionDelegateKt {
    ...
    public static final void m1670appendSelectableInfoParwq6A(SelectionLayoutBuilder selectionLayoutBuilder, TextLayoutResult textLayoutResult, long j, long j2, long j3) {
        Direction direction;
        ...
        int iM1671getOffsetForPosition3MmeM6k;
        int previousAdjustedOffset;
        Selection.AnchorInfo start;
        Selection.AnchorInfo end;
        Rect rect = new Rect(0.0f, 0.0f, IntSize.m6287getWidthimpl(textLayoutResult.getSize()), IntSize.m6286getHeightimpl(textLayoutResult.getSize()));
        Direction directionM1672getXDirection3MmeM6k = m1672getXDirection3MmeM6k(j, rect);
        Direction directionM1673getYDirection3MmeM6k = m1673getYDirection3MmeM6k(j, rect);
        if (selectionLayoutBuilder.getIsStartHandle()) {
            Selection previousSelection = selectionLayoutBuilder.getPreviousSelection();
            direction = directionM1673getYDirection3MmeM6k;
            direction2 = directionM1672getXDirection3MmeM6k;
        ...
        if (isSelected(SelectionLayoutKt.resolve2dDirection(direction2, direction), direction6)) {
            ...
            if (selectionLayoutBuilder.getIsStartHandle()) {
                previousAdjustedOffset = m1671getOffsetForPosition3MmeM6k(j, textLayoutResult);
                Selection previousSelection3 = selectionLayoutBuilder.getPreviousSelection();
                iM1671getOffsetForPosition3MmeM6k = (previousSelection3 == null || (end = previousSelection3.getEnd()) == null) ? previousAdjustedOffset : getPreviousAdjustedOffset(end, selectionLayoutBuilder.getSelectableIdOrderingComparator(), j4, length);
            } else {
                iM1671getOffsetForPosition3MmeM6k = m1671getOffsetForPosition3MmeM6k(j, textLayoutResult);
                Selection previousSelection4 = selectionLayoutBuilder.getPreviousSelection();
                previousAdjustedOffset = (previousSelection4 == null || (start = previousSelection4.getStart()) == null) ? iM1671getOffsetForPosition3MmeM6k : getPreviousAdjustedOffset(start, selectionLayoutBuilder.getSelectableIdOrderingComparator(), j4, length);
            }
            selectionLayoutBuilder.appendInfo(j4, previousAdjustedOffset, direction5, directionAppendSelectableInfo_Parwq6A$otherDirection, iM1671getOffsetForPosition3MmeM6k, direction3, direction4, OffsetKt.m3437isUnspecifiedk4lQ0M(j2) ? -1 : m1671getOffsetForPosition3MmeM6k(j2, textLayoutResult), textLayoutResult);
        }
    }
    ...
    private static final Direction m1672getXDirection3MmeM6k(long j, Rect rect) {
        return Offset.m3416getXimpl(j) < rect.getLeft() ? Direction.BEFORE : Offset.m3416getXimpl(j) > rect.getRight() ? Direction.AFTER : Direction.ON;
    }

    /* renamed from: getYDirection-3MmeM6k, reason: not valid java name */
    private static final Direction m1673getYDirection3MmeM6k(long j, Rect rect) {
        return Offset.m3417getYimpl(j) < rect.getTop() ? Direction.BEFORE : Offset.m3417getYimpl(j) > rect.getBottom() ? Direction.AFTER : Direction.ON;
    }
    ...
    private static final int m1671getOffsetForPosition3MmeM6k(long j, TextLayoutResult textLayoutResult) {
        if (Offset.m3417getYimpl(j) <= 0.0f) {
            return 0;
        }
        return Offset.m3417getYimpl(j) >= textLayoutResult.getMultiParagraph().getHeight() ? textLayoutResult.getLayoutInput().getText().length() : textLayoutResult.m5574getOffsetForPositionk4lQ0M(j);
    }
}
